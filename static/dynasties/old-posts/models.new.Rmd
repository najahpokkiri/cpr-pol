...
<!-- ```{r} -->
<!-- rmarkdown::pandoc_convert("D:/cpr/thesis/chaps_word/Chapter7_Aug2.docx", to = "latex", output = "D:/cpr/thesis/chaps_word/Chapter7_Aug2.tex")  -->
<!-- ``` -->
...
## Involvemnt
  ...
  # 
  # inv %>% group_by(family_id) %>% summarise(sum(org_inv, association_inv, poitical_inv))
  # 
  # 
  # inv_fam <- inv %>% distinct(family_id, .keep_all = TRUE) %>%select(family_id, pol_fam, association_fam, org_fam) 
  # 
  # 
  # inv_df <- left_join(dyn_ae_ge_fam_uniq, inv_fam, by = "family_id") %>% select(family_id, pol_fam, association_fam, org_fam) %>% mutate_all(~replace(., is.na(.), 0))
  # 
  # inv_df %>% summarise(mean(pol_fam), 
  #                      mean(association_fam),
  #                      mean(org_fam)) %>% 
  #   kable(digits =2, col.names = c("Political inv", "Associational inv", " Organisational inv"))
...
constituency <- dyn_ae_ge_uniq$pc_name
...
order = c("minister","ind_edu",  "land", "n_elections_w","mem1_female")
 ...
 addlines <- list(c('Fixed effects', '' ,"Decade", "Sub-region","Deacde  \\ Sub-reg"), c("Clustered SE" ,"PC", "PC", "PC","PC"), c("Caste control", "Yes","Yes","Yes","Yes"))
...
cov_labels <- c( "Political Diversification <br>(only cabinet)", "Economic diversification", "Land ownership",  "Elections won by first member","First member - female")
...
order = c("minister","ind_edu", "edu","n_elections_w","mem1_female")
 ...
 addlines <- list(c('Fixed effects', '' ,"Decade", "Sub-region","Deacde and Sub-region"), c("Clustered SE" , "PC", "PC", "PC", "PC"), c("Caste control", "Yes","Yes","Yes","Yes"))
...
sub_region <- dyn_ae_ge_09_19$sub_region
 ...
 constituency <-  dyn_ae_ge_09_19$pc_name
...
fit.1 <- felm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim   + log(total_assets)  +  serious_crime+turncoat|0|0|constituency, data = subset(df %>% filter(total_assets>0)))


fit.2<- felm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim    +log(total_assets)  +  serious_crime+turncoat|year|0|constituency, data = subset(df %>% filter(total_assets>0)))


fit.3 <- felm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim   +  log(total_assets)  +  serious_crime+turncoat|sub_region|0|constituency, data = subset(df %>% filter(total_assets>0)))



fit.4 <- felm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim   +   log(total_assets)  +  serious_crime+turncoat|year+sub_region|0|constituency,data = subset(df %>% filter(total_assets>0)))
...
addlines <- list(c('Fixed effects', '' ,"Year", "Sub-region","Year and Sub-region"), c("Clustered SE" ,"PC","PC","PC","PC"), c("Caste control", "Yes","Yes","Yes","Yes"))

cov_labels <- c( "Total assets (log)","Serious crime", "Turncoat")
...
fit.1 <- felm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim   +  log(total_assets)  +  serious_crime+turncoat|sub_region|0|constituency, data = subset(df %>% filter(total_assets>0 & n_el_w>=1)))

fit.2<- felm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim   +  log(total_assets)  +  serious_crime+turncoat|sub_region|0|constituency, data = subset(df %>% filter(total_assets>0 & n_el_w>=1)))


fit.3 <- felm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim   +  log(total_assets)  +  serious_crime+turncoat|sub_region|0|constituency, data = subset(df %>% filter(total_assets>0 & n_el_w>=1)))



fit.4 <- felm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim   +  log(total_assets)  +  serious_crime+turncoat|sub_region|0|constituency,data = subset(df %>% filter(total_assets>0 & n_el_w>=1)))
...
ac_map <- read.csv("D:/cpr/UP/up-dynasties/dyn_other_data/ac_pc/PC_AC_mapping.csv") %>%  
  rename_all(tolower) %>%filter(state_name == "Uttar_Pradesh")%>% select(constituency_no,pc_name)



pc_map <- read.csv("D:/cpr/UP/up-dynasties/dyn_other_data/ac_pc/PC_AC_mapping.csv") %>%  
  rename_all(tolower) %>%filter(state_name == "Uttar_Pradesh")%>% select(constituency_no= pc_no, pc_name) %>% distinct(constituency_no, .keep_all =TRUE)
...
dyn_ae <- readxl::read_xlsx("D:/cpr/UP/up-dynasties/dyn_other_data/TCPD_UP_VS_16_17_RK.xlsx")%>% 
  inner_join(ac_map, by =c("Constituency_No"= "constituency_no"))


#dyn_ae %>% rename_all(tolower)%>% select(1:1, 3:3, 4:4, 8:8, 10:14)


dyn_ae <- dyn_ae %>% select(3:4, 8:8, 12:12, pc_name)
...
dyn_ge <- readxl::read_xlsx("D:/cpr/UP/up-dynasties/dyn_other_data/UP_TCPD_GE_091419 _April_21.xlsx")%>% 
  inner_join(pc_map, by =c("Constituency_No"= "constituency_no"))




dyn_ge <-dyn_ge %>% select( 3:5,15:15, pc_name)
...
constituency <- dyn_ae_ge_adr$pc_name
...
df <-data.frame(winner ,   incumbent,  turncoat , female , terms , dynast , assets  , serious_crime,  year,enop, reservation_SCST,recontest, constituency, party, vote_share_pc, inc, bjp, margin_percentage,  position, turnout, sub_region,immovable_assets,movable_assets,mov_assets_ratio , caste_uc, caste_sc, caste_muslim, caste_obc, dynast_1_2, lagged_margin, lagged_turnout)
...
addlines <- list(c('Fixed effects', '' ,"Year", "Sub-region","Both"), c("Clustered SE" ,"PC","PC","PC","PC"), c("Caste control", "Yes","Yes","Yes","Yes"))
...
# 
# stargazer(fit.1, type = "html")



stargazer(fit.1,fit.2, fit.3,fit.4,dep.var.labels = c("Political Family"),title="Regression Results - AE-GE 09:19", type = "latex",
align=TRUE,  no.space=TRUE,add.lines=addlines, omit = drop,omit.stat=c("f", "ser"),
font.size = "small",column.sep.width = "-15pt", covariate.labels =  c("Assets (log)", "Serious crime", "Turncoat"))
