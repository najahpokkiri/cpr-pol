...
library(stargazer)
 ...
 stargazer(fit.length, fit.cost_km, fit.cost_project, title="Linear regression - PMGSY  ",type = "html",
...
- Castes with more than 11 observations


```{r}

raw_data %>% group_by(Caste) %>% summarise(count= n()) %>% filter(count>10 &Caste!= "NA") %>% arrange(-count) %>% kable() %>%  kable_styling(full_width = F)
