...
dyn_ae_ge$caste_groups <- factor(dyn_ae_ge$caste_groups, levels = c("Upper Caste","Yadav","Non-Yadav OBC","Dalit" , "Muslim"  ,"Others" ))
 ...
 sep <- dyn_ae_ge_fam_uniq %>% filter(caste_groups!= "NA") %>% group_by(success_cat, caste_groups) %>% summarise( count = n()) %>% group_by(success_cat) %>% mutate(sum = sum(count), prop = count/sum) %>% select(success_cat, caste_groups, prop)
...
stargazer(fit.surv, type="html", keep = keep,order = order, covariate.labels = labels,
          add.lines = lines,dep.var.labels = "Family life span",title = "Coax hazard regression")
...
stargazer(dead.surv1,dead.surv2, dead.surv3, dead.surv4,type = "html", keep = keep,order = order, covariate.labels = labels,
          ...
          header = FALSE,title = "Coax hazard regression", dep.var.labels.include = FALSE, dep.var.caption = "DV: Family life span")
