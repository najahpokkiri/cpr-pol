...
dyn_entry_exit %>% filter(is.na(entry_years))

dyn_ae_ge %>% filter(family_id =="f0005702" )
...
stargazer(fit.life, type="html
          ", keep = keep,order = order, covariate.labels = labels,
          add.lines = lines)
...
caste_all <- dyn_ae_ge %>% filter(dyn_cum_2 ==1) %>% distinct(family_id, .keep_all = TRUE)%>% group_by(caste_groups) %>% summarise(count = n(), prop = count/322 *100)%>% mutate(caste = "All") %>% 
...
caste_sep <- dyn_ae_ge_fam_uniq %>% filter(caste_groups!= "NA") %>% group_by(success_cat, caste_groups) %>% summarise( count = n()) %>% group_by(success_cat) %>% mutate(sum = sum(count), prop = count/sum) %>% select(success_cat, caste_groups, prop) %>% 
...
caste_all + caste_sep+
  plot_annotation(title = "Caste composition")
```
...
fit.multinom <- multinom(success_cat~ pol_div +  ind_edu +  land + entry_years +n_elections_w +caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim , data = fam_df2)
```

```{r}
 keep <- c("pol_div","success_cat","ind_edu", "land","entry_years", "n_elections_w")

order <- c("pol_div","success_cat","ind_edu",  "land", "years to entry", "n_elections_w")
...
| Cut off | 5 years(2013) | 10 years(2009) | 15 years(2005) |
|---------|---------|----------|----------|
| alive   | 128     | 110      | 97       |
| dead    | 130     | 90       | 66       |
| total   | 258     | 200      | 163      |
...
dead.surv3 <- coxph(Surv(fam_life, dead_15) ~  pol_div +minister + ind_edu + land + entry_years +n_elections_w+ caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim + factor(decade)  ,data = subset(fam_df3 %>% filter(min_fam_year<2005)))
 ...
 lines <-  list (c("Last ** years","5","10","5"),c("Caste control", "Yes","Yes", "Yes"), c("Decade control","Yes","Yes", "Yes"))
 
stargazer(dead.surv3, dead.surv1, dead.surv2,type = "html", keep = keep,order = order, covariate.labels = labels,
