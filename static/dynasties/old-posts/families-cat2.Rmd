...
based on the life span that we are using for survival analysis
...
So life span is one variable that takes an absolute difference between the year they first appeared and last appeared- this a flawed measure because this does not take the number of elections you have contested into consideration.We came with another metric to solve this - that a measure that is takes the terms that you have been active into account peaking at 50 years. (ie 1974 -2024 difference)

- the first plot uses the flawed measure and the second one uses the latest one which we have used in the survival models.
...
dyn_ae_ge_fam_uniq %>% group_by(caste_groups) %>% 
  summarise(mean(life_span),
            median(life_span)) %>% kable(digits = 2) %>% 
...
fam_span2%>% group_by(caste_groups) %>% 
  summarise(mean(fam_life),
            median(fam_life)) %>% kable(digits = 2) %>% 
  kable_styling(full_width = F)
...
df <- tibble(family_id,constituency,success_cat, caste_uc, caste_yadav,caste_non_yadav_obc , caste_dalit , caste_muslim ,minister_duration ,industry , land , linkage,linkage_down, minister, edu, second_mem_dur, delim_change, election_type, caste_groups, ind_edu)
