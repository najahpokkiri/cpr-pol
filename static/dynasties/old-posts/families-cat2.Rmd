...
```{r}
dyn_ae_ge_fam_uniq %>% filter(caste_groups!= "Others") %>% group_by(caste_groups) %>% summarise(mean = mean(life_span), median = median(life_span)) %>% kable(digits =0) %>% kable_styling(full_width = F)
```
...
# 
# ggplot(dyn_ae_ge_fam_uniq %>% filter(land<1000), aes(land))
# 
# 
# 
#   
...
df <- tibble(success_cat, caste_uc, caste_yadav,caste_non_yadav_obc , caste_dalit , caste_muslim ,minister_duration ,industry , land , linkage,linkage_down, minister, edu, second_mem_dur, delim_change)
...
fit.multinom <- multinom(success_cat~  caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry +  land + linkage+edu)
```

```{r, results= "asis"}


stargazer(fit.multinom, type="html", keep = c("linkage","success_cat","minister","industry", "land", "edu"),order = c("linkage","success_cat","minister","industry",  "land", "edu"),add.lines = list (c("Caste control", "Yes","Yes","Yes")),covariate.labels = c( "Political diversification","Minister", "Economic diversification", "Land ownership", "Educational inst.ownership"))
```
...
```{r}
df <- tibble( caste_uc, caste_yadav,caste_non_yadav_obc , caste_dalit , caste_muslim ,minister_duration ,industry , land , linkage,linkage_down, minister, edu, second_mem_dur, delim_change, serious_crime_avg, asset_avg, rising, stable)
...
fit.ols<- felm(rising~  caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry +  land + linkage+edu+serious_crime_avg +log(asset_avg) , data = df)

fit.ols1<- felm(stable~  caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry +  land + linkage+edu+serious_crime_avg +log(asset_avg) , data = df)
```

```{r, results= "asis"}


stargazer(fit.ols,fit.ols1, type="html", keep = c("linkage","success_cat","minister","industry", "land", "edu","serious_crime_avg","asset_avg"),order = c("linkage","success_cat","minister","industry",  "land", "edu","serious_crime_avg","asset_avg"),add.lines = list (c("Caste control", "Yes","Yes")),covariate.labels = c( "Political diversification","Minister", "Economic diversification", "Land ownership", "Educational inst.ownership", "Asset","Crime"))
```
