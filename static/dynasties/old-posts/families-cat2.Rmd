...
```{r}
mean_df <- Rmisc::summarySE(subset(fam_df %>% filter(success_cat!= "Rising")), measurevar = "pol_div", groupvars = c("land"))


mean_df$land <-  factor(mean_df$land)

ggplot(mean_df , aes(x=land, y=pol_div)) + 
    geom_bar(position=position_dodge(), stat="identity",
             colour="black", # Use black outlines,
            # size=.3
            ) +      # Thinner lines
    geom_errorbar(aes(ymin=pol_div-se, ymax=pol_div+se),
                  ...
                  position=position_dodge(.9)) +
    xlab("") +
    ylab("Mean political diversification") +
...
```{r}
mean_df <- Rmisc::summarySE(subset(fam_df %>% filter(success_cat!= "Rising")), measurevar = "econ_div", groupvars = c("land"))


mean_df$land <-  factor(mean_df$land)

ggplot(mean_df , aes(x=land, y=econ_div)) + 
    geom_bar(position=position_dodge(), stat="identity",
             colour="black", # Use black outlines,
             size=.3) +      # Thinner lines
    geom_errorbar(aes(ymin=econ_div-se, ymax=econ_div+se),
...
fam_life <-  dyn_ae_ge_fam %>%filter(success_cat!= "Rising") %>% 
 ...
 fam_df<- inner_join(fam, fam_df, by = "family_id")
...
Here we are running models on families a three different models for families that are started in those particular period in last 70 years. Aim of these models is to show that variables are still relavant irrespective of the families commencenmt year and age
...
dead.surv2 <- coxph(Surv(fam_life, dead_5) ~  pol_div +minister + ind_edu + land + entry_years +n_elections_w +  caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim  + factor(decade) ,  data = subset(fam_df %>% filter(min_fam_year<2014 &success_cat!= "Rising")))
...
```{r}
 ...
 ggind_edu_all <- ggsurvplot(survfit(Surv(fam_life2, dead) ~ind_edu_cat , data = subset(fam_df %>% filter(success_cat!= "Rising"))), 
     ...
     #ylab = "Overall survival probability",
     ggtheme = theme_light(),
     subtitle = " wrt economic diversification",
     #title = "Survival of political families wrt economic diversification",
...
```{r}
 ...
 pol_div_all <- ggsurvplot(survfit(Surv(fam_life2, dead) ~pol_div1 , data = subset(fam_df %>% filter(success_cat!= "Rising"))),  
     ...
     ylab = "Overall survival probability",
       xlab = "Years", 
     ggtheme = theme_light(),
     subtitle = " wrt political diversification",
     #title = "Survival of political families wrt economic diversification",
   legend.labs = c("low",  "high")
  )
