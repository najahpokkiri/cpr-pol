...
$${Life \ span \ of \ family} = \alpha +\beta_{1}Political \ diversification   {_i}   +   \beta_{2} Economic  \ diversification{_i}  +  \\ \beta_{3}Land \ ownership{_i} + \beta_{3} Years \ to \ second\\ member \ entry{_i} + \\ \beta_{3} Election \ won \ by  \ first\  member{_i}  + Z +  \epsilon  $$
...
fam_all <- inner_join(fam_life_all,fam_life_2, by = "family_id")
 ...
 fam_all <- fam_all %>% mutate(
                ...
                dead_20 = ifelse(max_year>2000,1,2)
 )
...
dead.surv2 <- coxph(Surv(fam_life, dead_5) ~  pol_div +minister + ind_edu + land + entry_years +n_elections_w +  caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim  + factor(decade) ,  data = subset(fam_df3 %>% filter(min_fam_year<2013)))

dead.surv3 <- coxph(Surv(fam_life, dead_15) ~  pol_div +minister + ind_edu + land + entry_years +n_elections_w+ caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim + factor(decade)  ,data = subset(fam_df3 %>% filter(min_fam_year<2005)))


dead.surv4 <- coxph(Surv(fam_life, dead_20) ~  pol_div +minister + ind_edu + land + entry_years +n_elections_w+ caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim + factor(decade)  ,data = subset(fam_df3 %>% filter(min_fam_year<2001)))


 lines <-  list (c("Last ** years","10","5","10","15"),c("Caste control", "Yes","Yes", "Yes", "Yes"), c("Decade control","Yes","Yes", "Yes", "Yes"))
