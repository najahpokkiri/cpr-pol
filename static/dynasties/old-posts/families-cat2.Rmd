...
ggplot(subset(rt %>% filter(success_cat != "Rising")) ,aes(x = success_cat, y = prop, fill = indus_rt, label = round(prop,2)))+
  geom_bar(position = "stack", stat= "identity")+
      geom_text(size = 2.5, position = position_stack(vjust = 0.5))+
    colorspace::scale_fill_discrete_qualitative(palette= "dark3" )+
  coord_fixed(ratio =4)+
    theme_minimal()+
labs(title = "Industry composition  ",x = "", y = "Proportion of industry")+
    theme(plot.margin = unit(c(.5,.5,.5,.5), "cm"))+
  theme(plot.background = element_blank(),
        axis.line.x = element_line(),
        panel.grid.major = element_blank(),
...
mean_df_pol2 <- Rmisc::summarySE(dyn_ae_ge_uniq1, measurevar = "pol_div", groupvars = c("success_cat1"))%>% mutate(div = "Political diversification")
...
ggplot(econ_pol_df2, aes(x=success_cat1, y=pol_div)) + 
  ...
  # scale_y_continuous(breaks = c(0.0,1.0,2.0), labels = scales::number_format(accuracy = 0.1))+
    xlab("") +
    ylab("") +
...
all_df <- dyn_ae_ge_uniq1 %>% summarise(mean = mean(econ_div),
                                       se = std(econ_div))  %>% mutate(cat = "All entities")

# mean_df$success_cat <- factor(mean_df$success_cat, levels = c("Non_fam", "Rising", "Stable"))
ggplot(all_df, aes(x=cat, y=mean)) + 
...
fam_df %>% group_by(success_cat) %>% summarise(lnkage = mean(linkage),
...
ggplot( aes(variable, y = value, fill = success_cat, label = round(value,2)))+
  ...
  #coord_fixed(ratio =5)+
    ...
    labs(title = "Political diversification wrt family class", subtitle = "Proportion of families") +
...
<!-- ```{r} -->
<!-- mean_df_pol <- Rmisc::summarySE(subset(fam_df %>% filter(success_cat!= "Rising")), measurevar = "pol_div", groupvars = c("land")) %>% mutate(cat = "Political diverisfication") %>% rename(  div = pol_div) -->


<!-- mean_df_econ <- Rmisc::summarySE(subset(fam_df %>% filter(success_cat!= "Rising")), measurevar = "econ_div", groupvars = c("land"))%>% mutate(cat = "Economic diversification") %>% rename(div = econ_div) -->
...
<!-- pol_div_land <- ggplot(mean_df , aes(x=land, y=div)) +  -->
<!--     geom_bar(position=position_dodge(), stat="identity", -->
<!--              colour="black", # Use black outlines, -->
<!--             # size=.3 -->
<!--             ) +      # Thinner lines -->
...
<!--     #                breaks=c("OJ", "VC"), -->
<!--     #                labels=c("Orange juice", "Ascorbic acid")) ++ -->
<!--   scale_x_discrete(labels = c("Very small", "Small", "Medium", "Large", "Very large"))+ -->
<!--     theme_bw() -->
...
```{r}
ggplot(dyn_entry_exit, aes(entry_years))+
  geom_density(size = 1, color = 'black')+
  labs(subtitle = "Years to second member entry", x = "Years",y = "Density")+
  #xlim(0,45)
  ylim(.0,.25)+
...
stargazer(fit.life, type="html", keep = keep,order = order, covariate.labels = labels,
          add.lines = lines, dep.var.labels = "Family life span")
...
all_sep <- rbindlist(list(all, sep), use.names = TRUE)
  ...
  ggplot(all_sep, aes(factor(success_cat), prop, fill = caste_2,label = ifelse(prop>.04,round(prop,2),"")))+
   geom_bar(stat="identity", position = "stack")+
    geom_text(size = 2.5, position = position_stack(vjust = 0.5))+ 
  colorspace::scale_fill_discrete_qualitative(palette= "dark3" )+
  #coord_fixed(ratio =5)+
...
# 
# ae/ge
ggplot(adr_non_fam %>% filter(year!= 2009), aes(success_cat, log(total_assets)))+
  geom_boxplot(outlier.shape = NA)+
  facet_grid(~year)+
  theme_light()+
  theme(
...
stargazer(fit.declining,  type="html",
          keep = keep, order = order,
          add.lines = lines,
          covariate.labels = labels,
...
stargazer(fit.surv, type="html", keep = keep,order = order, covariate.labels = labels,
...
```{r}
 ...
 ### decades
   ...
   pol_div_all$plot+ggind_edu_all$plot+
    plot_annotation(
    title = "Survival of political families"
  )+
    plot_layout(guides = "collect") & theme(legend.position = "bottom")
