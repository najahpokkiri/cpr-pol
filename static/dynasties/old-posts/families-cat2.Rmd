...
<!-- adr_ae$total_assets <- DescTools::Winsorize(as.numeric(adr_ae$total_assets), na.rm = TRUE,probs = c(.01,.99)) -->

<!-- adr_ae$total_movable_assets_totals <- DescTools::Winsorize(as.numeric(adr_ae$total_movable_assets_totals ), na.rm = TRUE,probs = c(.01,.99)) -->


<!-- adr_ae$total_immovable_assets_totals <- DescTools::Winsorize(as.numeric(adr_ae$total_immovable_assets_totals ), na.rm = TRUE,probs = c(.01,.99)) -->
...
<!-- caste_uc <- ifelse(dyn_ae_adr$caste_rec%in% c("IC","UC","uc"),1,0) -->

<!-- caste_muslim <- as.numeric(case_when(grepl( "muslim",dyn_ae_adr$caste_rec, ignore.case = TRUE) ~"1", -->
<!--                    TRUE ~"0")) -->


<!-- caste_sc <- ifelse(dyn_ae_adr$caste_rec %in% c("SC","ST"),1,0) -->

<!-- caste_obc<- ifelse(dyn_ae_adr$caste_rec =="OBC",1,0) -->
...
<!-- adr_dyn_fam <- adr_ae %>% rename_all(tolower)%>% filter(position %in%c(1,2)) %>% distinct(year, constituency_no, position, .keep_all = TRUE) %>% inner_join((dyn_ae_ge %>% select(year, constituency_no, position, family_id, relative_id, rel_id_rep)), by = c("year","constituency_no", "position")) %>% inner_join(fam_df, by = "family_id") -->
<!-- ``` -->
...
fit.stable <- felm(stable~ pol_div +  ind_edu +  land + entry_years +n_elections_w +caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim , data = subset(fam_df %>% filter(success_cat!= "Rising")))
...
stargazer(fit.declining, fit.stable, type="html",
          keep = keep, order = order,
          add.lines = lines,
          covariate.labels = labels)
