...
fisrt_member <- dyn_ae_ge_fam %>% group_by(family_id) %>% filter(year== min(year)) %>% arrange(family_id, relative_id) %>% distinct(family_id, .keep_all = TRUE) %>% select(family_id, first_member = relative_id) %>% mutate(first_member = ifelse(is.na(first_member),4,first_member))
...
fit.multinom <- multinom(success_cat~  minister +ind_edu +  land + pol_div + entry_years +n_elections_w +caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim , data = fam_df2)
```

```{r}
 keep <- c("pol_div","success_cat","minister","ind_edu", "land","entry_years", "n_election_w")

order <- c("pol_div","success_cat","minister","ind_edu",  "land", "years to entry", "n_election_w")


 lines <-  list (c("Caste control", "Yes","Yes","Yes"))
 
 labels <-  c( "Political diversification","Minister", "Economic diversification", "Land ownership", "Years to second member entry", "Elections won by founder")
```
...
```{r}
 ...
 pol_div_all <- ggsurvplot(survfit(Surv(fam_life, dead) ~pol_div , data = fam_df1), 
     xlab = "", 
     #ylab = "Overall survival probability",
     ggtheme = theme_light(),
     subtitle = "All families",
     #title = "Survival of political families wrt economic diversification",
  #legend.labs = c( "very small", "small", "medium", "large", "very large")
  )
 ...
 ### decades
    ...
    pre_80<- ggsurvplot(survfit(Surv(fam_life, dead) ~pol_div , data = subset(fam_df1 %>% filter(min_year<1980))), 
     xlab = "", 
     ylab = "",
     ggtheme = theme_light(),
     #title = "Survival of political families wrt economic diversification",
     ...
     #legend.labs = c("Pol diversification - NO", "Pol diversification - YES")
      ...
      pre_90 <- ggsurvplot(survfit(Surv(fam_life, dead) ~pol_div , data = subset(fam_df1 %>% filter(min_year<1990))), 
     xlab = "Years", 
  ...
  pre_99 <- ggsurvplot(survfit(Surv(fam_life, dead) ~pol_div , data = subset(fam_df1 %>% filter(min_year<1999))), 
     xlab = "Years", 
     ylab = "",
     ggtheme = theme_light(),
     #title = "Survival of political families wrt economic diversification",
...
fam_all <- inner_join(fam_life_all,fam_life_2, by = "family_id")
 ...
 edu_dummy1 <- dyn_ae_ge_fam %>% distinct(family_id, .keep_all = TRUE) %>% 
  ...
  fam_df2 <- fam_df1 %>% select(-c(fam_life,min_year, max_year, decade)) %>% inner_join(fam_all,  by = "family_id")
...
dead.surv1 <- coxph(Surv(fam_life, dead_10) ~  pol_div +minister + ind_edu + land + entry_years +n_elections_w + caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim + factor(decade)  , data = subset(fam_df2 %>% filter(min_fam_year<2009)))



dead.surv2 <- coxph(Surv(fam_life, dead_5) ~  pol_div +minister + ind_edu + land + entry_years +n_elections_w +  caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim  + factor(decade) ,  data = subset(fam_df2 %>% filter(min_fam_year<2013)))

dead.surv3 <- coxph(Surv(fam_life, dead_15) ~  pol_div +minister + ind_edu + land + entry_years +n_elections_w+ caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim + factor(decade)  ,data = subset(fam_df2 %>% filter(min_fam_year<2005)))


 lines <-  list (c("Years","last 10 years","last 5 years","last 15 years"),c("Caste control", "Yes","Yes", "Yes"), c("Decade control","Yes","Yes", "Yes"))
 
stargazer(dead.surv1, dead.surv3, dead.surv2,type = "html", keep = keep,order = order, covariate.labels = labels,
