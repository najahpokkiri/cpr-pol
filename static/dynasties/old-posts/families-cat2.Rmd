...
fam_df2 <-  inner_join(fam_df, life_span_df, by = "family_id") %>% inner_join(inv_df, by = "family_id")

fam_df2 <- fam_df2 %>% rowwise() %>% mutate(pol_div = minister + org_fam +linkage ) %>% ungroup()


fit.life <- felm(fam_life ~  pol_div +ind_edu +  land +entry_years + n_elections_w+ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim ,data = fam_df2)
...
fam_df2 <- fam_df2 %>% mutate(pol_div1= ifelse(pol_div %in% c(2,3), 3, pol_div),
                              pol_div1 = factor(pol_div1, levels = c(3,2,1,0)))

fam_df2 %>% group_by(success_cat) %>% summarise(lnkage = mean(linkage),
...
fam_df2$success_cat<-factor( fam_df2$success_cat,levels = c( "Declining","Stable","Rising"))



fit.multinom <- multinom(success_cat~ pol_div +  ind_edu +  land + caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim , data = fam_df2)
```
...
```{r message = FALSE, warning = FALSE, echo = FALSE}
fam_df2$success_cat<-factor( fam_df2$success_cat,levels = c( "Stable","Rising","Declining"))
...
```{r message = FALSE, warning = FALSE, echo = FALSE}
fam_df2$success_cat<-factor( fam_df2$success_cat,levels = c( "Rising","Declining","Stable"))
...
```{r, results="asis"}
fam_df2$stable<-ifelse(fam_df2$success_cat=="Stable", 1,0)

fit.stable <- felm(stable~ pol_div +  ind_edu +  land + entry_years +n_elections_w +caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim , data = fam_df2)

fam_df2$rising<-ifelse(fam_df2$success_cat=="Rising", 1,0)

fit.rising <- felm(rising~ pol_div +  ind_edu +  land + entry_years +n_elections_w +caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim , data = fam_df2)

fam_df2$declining<-ifelse(fam_df2$success_cat=="Declining", 1,0)

fit.declining <- felm(declining~ pol_div +  ind_edu +  land + entry_years +n_elections_w +caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim , data = fam_df2)
...
fam_df1 <-  fam_df2 %>% select(entry_years, pol_div, family_id) %>% inner_join(fam_df1, by = "family_id")
...
fam_df1 <- fam_df1 %>% mutate(pol_div1= ifelse(pol_div %in% c(2,3), 3, pol_div))

 pol_div_all <- ggsurvplot(survfit(Surv(fam_life, dead) ~pol_div1 , data = fam_df1), 
     xlab = "", 
     #ylab = "Overall survival probability",
     ggtheme = theme_light(),
     subtitle = "All families",
 ...
 ### decades
    ...
    pre_80<- ggsurvplot(survfit(Surv(fam_life, dead) ~pol_div1 , data = subset(fam_df1 %>% filter(min_year<1980))), 
     xlab = "", 
     ylab = "",
     ggtheme = theme_light(),
     #title = "Survival of political families wrt economic diversification",
     subtitle = "Pre-1980",
      ...
      pre_90 <- ggsurvplot(survfit(Surv(fam_life, dead) ~pol_div1 , data = subset(fam_df1 %>% filter(min_year<1990))), 
     xlab = "Years", 
  ...
  pre_99 <- ggsurvplot(survfit(Surv(fam_life, dead) ~pol_div1 , data = subset(fam_df1 %>% filter(min_year<1999))), 
     xlab = "Years", 
     ylab = "",
...
fam_df1 <- fam_df2 %>% select(-fam_life) %>% inner_join(fam, by = "family_id")

fit.surv <- coxph(Surv(fam_life, dead) ~ pol_div +minister + ind_edu + land + entry_years +n_elections_w+ caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim , data = fam_df1)
...
```{r results = "asis"}
fit.surv1 <- coxph(Surv(fam_life, dead) ~pol_div +minister + ind_edu + land + entry_years +n_elections_w + caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim + factor(decade) , data = fam_df1)



fit.surv2 <- coxph(Surv(fam_life, dead) ~ pol_div +minister + ind_edu + land + entry_years +n_elections_w+ caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim + factor(decade) , data = subset(fam_df1 %>% filter(min_year>1979)))

fit.surv3 <- coxph(Surv(fam_life, dead) ~ pol_div +minister + ind_edu + land + entry_years +n_elections_w + caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim + factor(decade) , data = subset(fam_df1 %>% filter(min_year>1985)))
...
fam_all <- inner_join(fam_life_all,fam_life_2, by = "family_id")
 ...
 edu_dummy1 <- dyn_ae_ge_fam %>% distinct(family_id, .keep_all = TRUE) %>% 
  ...
  fam_df3 <- fam_df2 %>%  inner_join(fam_all,  by = "family_id")
  
  fam_df3 <- inner_join(fam_df3, edu_dummy1, by = "family_id")
...
```{r results = "asis"}
# 
fam_df3 <- fam_df3 %>% select(-ind_edu.x, ind_edu = ind_edu.y, fam_life = fam_life.y)


dead.surv1 <- coxph(Surv(fam_life, dead_10) ~  pol_div +minister + ind_edu + land + entry_years +n_elections_w + caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim + factor(decade)  , data = subset(fam_df3 %>% filter(min_fam_year<2009)))



dead.surv2 <- coxph(Surv(fam_life, dead_5) ~  pol_div +minister + ind_edu + land + entry_years +n_elections_w +  caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim  + factor(decade) ,  data = subset(fam_df3 %>% filter(min_fam_year<2014)))

dead.surv3 <- coxph(Surv(fam_life, dead_15) ~  pol_div +minister + ind_edu + land + entry_years +n_elections_w+ caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim + factor(decade)  ,data = subset(fam_df3 %>% filter(min_fam_year<2004)))


dead.surv4 <- coxph(Surv(fam_life, dead_20) ~  pol_div +minister + ind_edu + land + entry_years +n_elections_w+ caste_uc + caste_yadav + caste_non_yadav_obc+caste_dalit+caste_muslim + factor(decade)  ,data = subset(fam_df3 %>% filter(min_fam_year<1999)))
