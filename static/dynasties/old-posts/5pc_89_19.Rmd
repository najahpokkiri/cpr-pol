...
tcpd_dyn5 <- inner_join(pc5, ge_all, by = c("state_name", "year","constituency_no","position")) %>% filter(position %in% c(1,2))
...
## Family politicians - year by year

```{r results="asis", warning=FALSE}


tcpd_dyn5%>%select(year, dyn) %>%  group_by(year) %>% summarytools::dfSummary( plain.ascii  = FALSE, 
          style        = "grid", 
          graph.magnif = 0.75, 
          valid.col    = FALSE,
          headings = FALSE,
...
tcpd_dyn5 <- tcpd_dyn5 %>% mutate(dynast = ifelse(dyn==1,1,0))

tcpd_dyn5 %>% group_by(year,position, dynast) %>%summarise(count = n()) %>%
  group_by(year, position) %>% 
  mutate(sum = sum(count),
         prop = count/sum) %>% 
  ggplot(aes(factor(year), prop, fill = factor(dynast), label = round(prop,2)))+
  geom_bar(stat = "identity", position = "stack")+
 geom_text(size = 2.5, position = position_stack(vjust = 0.5))+
      theme_minimal()+

  scale_fill_grey(labels = c( "Non-Family","Family"))+
   labs(title = "Family Politicians in Loksabha",
         subtitle = "Year & position snapshot"
) +
  facet_wrap(~position)+
...
```{r}
states <- tcpd_dyn5 %>% group_by(state_name, dynast) %>%summarise(count = n()) %>% 
  mutate(sum = sum(count),
         prop = count/sum) %>% filter(sum>80 & !is.na(state_name) & state_name != "NA") 
 
 
  order <- tcpd_dyn5 %>% group_by(state_name, dynast) %>%summarise(count = n()) %>% 
  mutate(sum = sum(count),
         prop = count/sum) %>% filter(sum>80 & dynast ==1&!is.na(state_name) & state_name != "NA") %>%arrange(prop) %>%  magrittr::extract2("state_name")
  
  states <- states %>% mutate(state_name= factor(state_name, order))
  
  ggplot(states, aes(y =state_name, x =prop, fill = factor(dynast), label = round(prop,2)))+
...
# geom_text(size = 2.5, position = position_stack(vjust = 0.5))+
  ...
  scale_fill_grey(labels = c( "Non-Family","Family"))+
   labs(title = "Family Politicians in Loksabha",
         subtitle = "State by state snapshot"
) +
  theme(plot.background = element_blank(),
        axis.line.x = element_line(),
        panel.grid.major = element_blank(),
