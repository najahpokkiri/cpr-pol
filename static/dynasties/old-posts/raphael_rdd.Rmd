...
ggplot(cv %>% filter(position %in% c(1,2)) , aes( vote_share_percentage,cv, color = factor(dyn)))+
 #geom_point()+
  ...
  # xlim(0,90)+
  scale_color_discrete(labels = c("Non-fam","Family"))+
  labs(x = "vote share", y = "CoV", title = "Coeficient of varation as a function of vote share - winner a& runner up")+
  theme_minimal_hgrid()+
  theme(legend.position = "bottom",
        legend.title = element_blank())
...
rdd <- cv_1  %>% filter(dyn_vs_nondyn==1&margin_percentage<=5 &position==1) %>% mutate(mv = ifelse(dyn==0,-margin_percentage, margin_percentage)) %>% select(state_name,year_x,constituency_no,dyn,margin_percentage,mv,cv)
