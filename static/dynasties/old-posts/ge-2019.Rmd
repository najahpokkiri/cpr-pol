...
adr1 <- fread("D:/cpr/UP/up-dynasties/dyn_other_data/adr/adr_tcpd_09_14_19.csv") %>% filter(Vote_Share_Percentage>5 & !is.na(Year)& Year!= "") %>% distinct(StateName, Year, Constituency_No, Position, .keep_all = TRUE) %>% mutate(StateName = tolower(StateName))
...
adr_ks <- fread("D:/cpr/UP/up-dynasties/dyn_other_data/adr/ls_adr_kshirin2.csv") %>% 
 mutate(constituency_name = tolower(constituency_name),
         candidate_name = tolower(candidate_name), 
        candidate_name = str_trim(candidate_name),
        state_name = tolower(state_name)) 


adr_ks <- inner_join(adr_ks, dyn_ge, by = c("state_name"= "State_Name", "year"="Year", "constituency_no"="Constituency_No", "position" ="Position"))



  adr_ks_match<- inner_join(adr_ks, adr2, by = c("year"="Year","state_name" = "StateName","constituency_no"="Constituency_No","position"="Position")) 
...
# 
# 
# adr_ks_match <- adr_ks_match %>% group_by(state_name, year,constituency_name, position) %>% mutate(rep = n()) %>% distinct(state_name, year,constituency_name, position, .keep_all = TRUE)

# 
  
#adr1$State_Name <- NULL
...
names(adr_ks_match) <- tolower(names(adr_ks_match))


dyn_ge_adr<-rbindlist(list(adr_ks_match,dyn_ge_adr), use.names = TRUE, fill = TRUE)
...
df$constituency_id <- paste0(df$state,df$constituency)
...
fit_ac <- feglm(winner ~   incumbent+  turncoat +caste_UC_IC+ female + terms + dynast + log(assets)  + serious_crime+ education+age + inc + bjp| state+year+constituency_id, family = binomial(link = "probit" ), data = df )


fit_party <- feglm(winner ~   incumbent+  turncoat +caste_UC_IC+ female + terms + dynast + log(assets)  + serious_crime+ education+age | state+year+constituency_id, family = binomial(link = "probit" ), data = df )
...
fit_ac<- felm(vote_share_pc ~   incumbent+  turncoat +caste_UC_IC+ female+ terms + dynast + log(assets)  + serious_crime+ education+age+ inc + bjp | year+state+constituency_id|0|constituency+state, data = subset(df %>% filter(assets>0)) )
...
fit.1 <- felm(turnout~  dynast_1_2+ reservation_SCST + margin_percentage+enop+criminal_1_2|0|0|state, data = subset(df %>% distinct(year, state, constituency,.keep_all =TRUE)))



fit.2 <- felm(turnout~  dynast_1_2+ reservation_SCST + margin_percentage+enop+criminal_1_2|year|0|state, data = subset(df %>% distinct(year, state, constituency,.keep_all =TRUE)))

fit.3 <- felm(turnout~  dynast_1_2+ reservation_SCST + margin_percentage+enop+criminal_1_2|state|0|state,data = subset(df %>% distinct(year, state, constituency,.keep_all =TRUE)))

fit.4<- felm(turnout~  dynast_1_2+ reservation_SCST + margin_percentage+enop+criminal_1_2| state+year|0|state, data = subset(df %>% distinct(year, state, constituency,.keep_all =TRUE)))




fit.5<- felm(turnout~  dynast_1_2+ reservation_SCST + margin_percentage+enop+criminal_1_2|constituency_id+ state+year|0|state,data = subset(df %>% distinct(year, state, constituency,.keep_all =TRUE)))
...
fit.1 <- felm(turnout~  dynast_1_2+ reservation_SCST + margin_percentage+enop+criminal_1_2+lagged_turnout|0|0|state,data = subset(df %>% distinct(year, state, constituency,.keep_all =TRUE)))



fit.2 <- felm(turnout~  dynast_1_2+ reservation_SCST + margin_percentage+enop+criminal_1_2+lagged_turnout|year|0|state,data = subset(df %>% distinct(year, state, constituency,.keep_all =TRUE)))

fit.3 <- felm(turnout~  dynast_1_2+ reservation_SCST + margin_percentage+enop+criminal_1_2+lagged_turnout|state|0|state,data = subset(df %>% distinct(year, state, constituency,.keep_all =TRUE)))

fit.4<- felm(turnout~  dynast_1_2+ reservation_SCST + margin_percentage+enop+criminal_1_2+lagged_turnout| state+year|0|state,data = subset(df %>% distinct(year, state, constituency,.keep_all =TRUE)))




fit.5<- felm(turnout~  dynast_1_2+ reservation_SCST + margin_percentage+enop+criminal_1_2+lagged_turnout|constituency_id+ state+year|0|state, data = subset(df %>% filter(position==1)))
...
fit.1 <- felm(margin_percentage~  dynast_1_2+ reservation_SCST + turnout+enop|0|0|state, data = subset(df %>% filter(position==1)))



fit.2 <- felm(margin_percentage~  dynast_1_2+ reservation_SCST +turnout+enop|year|0|state, data = subset(df %>% filter(position==1)))

fit.3 <- felm(margin_percentage~  dynast_1_2+ reservation_SCST +turnout+enop|state|0|state, data = subset(df %>% filter(position==1)))

fit.4<- felm(margin_percentage~  dynast_1_2+ reservation_SCST + turnout+enop| state+year|0|state, data = subset(df %>% filter(position==1)))




fit.5<- felm(margin_percentage~  dynast_1_2+ reservation_SCST + turnout+enop|constituency_id+ state+year|0|state, data = subset(df %>% filter(position==1)))


addlines <- list(c('Fixed effects', '' ,"Year", "State","Year and State","State, Year and constituency_id"), c("Clustered SE" ,"State","State","State","State", "State"))
...
# 
# fit_ac <- feglm(dynast_winner ~  margin_percentage+reservation_SCST + turnout| state+year+constituency, family = binomial(link = "probit" ), data = df)
# 
# 
...
# 
# 
# fit_lm <- felm(dynast_winner ~  margin_percentage+ incumbent+  turncoat +caste_UC_IC+ female + terms + log(assets)  + serious_crime+ education+age + inc + bjp| factor(state)+factor(year)+factor(constituency),data =df_win)
```
...
```{r results="asis"}
stargazer(fit_dyn, fit_uc,fit_incumbent, fit_asset,fit_crime, type = "html", add.lines = list(c("Fixed effects", "State+Year+Constituency","State+Year+Constituency", "State+Year+Constituency" ,"State+Year+Constituency","State+Year+Constituency")),c("Clustered SE","Constituency","Constituency","Constituency","Constituency","Constituency"))
...
fit_ac <- feglm(winner ~   incumbent+  turncoat +caste_UC_IC+ female + terms + dynast + log(assets)  + serious_crime+ education+age +inc +bjp|year+constituency_id, family = binomial(link = "probit" ), data = df )
...
fit_ac <- felm(vote_share_pc ~   incumbent+  turncoat +caste_UC_IC+ female + terms + dynast + log(assets)  + serious_crime+ education+age+inc+bjp | year+constituency_id|0|constituency_id, data = df )
