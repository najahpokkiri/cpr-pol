...
dyn_ge %>% group_by(state_name, year ) %>% summarise(count = n(),
  ...
  filter(count>17) %>%select(state_name, year,prop_dyn_1_2) %>% filter(state_name %!in% c("jammu & kashmir","jharkhand") )%>% 
...
fit.5<- felm(turnout~  dynast_1_2+ reservation_SCST + margin_percentage+enop+criminal_1_2|constituency+ state+year|0|state, data = subset(df %>% filter(position==1)))
...
fit.5<- felm(turnout~  dynast_1_2+ reservation_SCST + margin_percentage+enop+criminal_1_2+lagged_turnout|constituency+ state+year|0|state, data = subset(df %>% filter(position==1)))
...
fit.5<- felm(margin_percentage~  dynast_1_2+ reservation_SCST + turnout+enop|constituency+ state+year|0|state, data = subset(df %>% filter(position==1)))
...
fit.5<- felm(margin_percentage~  dynast_1_2+ reservation_SCST + turnout+enop+lagged_margin|constituency+ state+year|0|state, data = subset(df %>% filter(position==1)))
...
```{r}
fit_dyn <- felm(dynast_winner ~  margin_percentage+reservation_SCST + turnout+enop| state+year+constituency|0|constituency, data = df)


fit_uc <- felm(uc_ic_winner~  margin_percentage+ reservation_SCST + turnout+enop| state+year+constituency|0|constituency,  data = df)


fit_incumbent<- felm(incumbent_winner~ margin_percentage+reservation_SCST + turnout+enop| state+year+constituency|0 |constituency,  data = df)


fit_asset <- felm(asset_winner~ margin_percentage+ reservation_SCST + turnout+enop| state+year+constituency|0|constituency, data = df)


fit_crime<- felm(ser_crime_winner~ margin_percentage+reservation_SCST + turnout+enop| state+year+constituency|0|constituency, data = df)
