...
lagged_turnout <- dyn_ge_adr$lagged_turnout
 ...
 dynast_1_2 <- dyn_ge_adr %>% group_by(state_name,year, constituency_no) %>% mutate(dynast_1_2 =ifelse( position %in% c(1,2) & any( dyn==1),1,0)) %>%ungroup()%>% select(dynast_1_2)
 
 
 dynast_cand <- dyn_ge_adr %>% group_by(state_name,year, constituency_no) %>% mutate(dynast_cand=ifelse( any( dyn==1),1,0)) %>%ungroup()%>% select(dynast_cand)
...
```{r}
mean <- df %>%distinct(year, constituency,position, .keep_all = TRUE) %>% summarise_at( c("winner"  , "incumbent", "turncoat" ,"female" , "terms", "dynast", "assets", "serious_crime", "education","age" , "reservation_SCST", "caste_UC_IC"),~mean(., na.rm = TRUE))

min <-  df %>%distinct(year, constituency,position, .keep_all = TRUE)  %>% summarise_at( c("winner"  , "incumbent", "turncoat" ,"female" , "terms", "dynast", "assets", "serious_crime", "education","age" , "reservation_SCST", "caste_UC_IC"),~min(., na.rm = TRUE))

max <-  df %>%distinct(year, constituency,position, .keep_all = TRUE)  %>% summarise_at( c("winner"  , "incumbent", "turncoat" ,"female" , "terms", "dynast", "assets", "serious_crime", "education","age" , "reservation_SCST", "caste_UC_IC"),~max(., na.rm = TRUE))

unique_n<-  df %>%distinct(year, constituency,position, .keep_all = TRUE)  %>% summarise_at( c("winner"  , "incumbent", "turncoat" ,"female" , "terms", "dynast", "assets", "serious_crime", "education","age" , "reservation_SCST", "caste_UC_IC"),list(n = "uniqueN"))

n<-  df %>%distinct(year, constituency,position, .keep_all = TRUE) %>% summarise_at( c("winner"  , "incumbent", "turncoat" ,"female" , "terms", "dynast", "assets", "serious_crime", "education","age" , "reservation_SCST", "caste_UC_IC"),list(n = "length"))


sd<-  df %>%distinct(year, constituency,position, .keep_all = TRUE)  %>% summarise_at( c("winner"  , "incumbent", "turncoat" ,"female" , "terms", "dynast", "assets", "serious_crime", "education","age" , "reservation_SCST", "caste_UC_IC"),list(sd= "sd"))
...
# 
# dyn_ge  %>% ungroup()  %>%  group_by(year ) %>% summarise(count = sum(position==1),
#                                              dyn = sum(dyn[position==1]),
#                                              prop_dyn = dyn/count,
#                                              count_1_2 = sum(ifelse(dyn==1& position==2,1,0)) )
# 
# dyn_ge  %>% ungroup()  %>%  group_by(year ) %>% summarise(count = sum(position==1),
#                                              dyn = sum(dyn[position==1]),
#                                              prop_dyn = dyn/count,
#                                              count_1_2 = sum(ifelse(dyn==1& position==2,1,0)) ,
#                                               dyn_1_2 = sum(dyn[position %in% c(1,2)], na.rm =TRUE),
...
dyn_ge %>% mutate(pos = ifelse(position >2,"3+",position))%>% group_by(year, pos ,dyn) %>% summarise(count = n()) %>% group_by(year, pos) %>% 
  mutate(total = sum(count), prop = count/total) %>% filter(dyn==1)%>% ungroup() %>% select(-dyn) %>% 
  select(year, pos, prop) %>% 
  pivot_wider(values_from = "prop", names_from ="year") %>% 
  kable(digits =2) %>% 
  kable_styling(full_width = F)
...
dyn_ge  %>% ungroup()  %>% filter(position %in% c(1,2) ) %>% group_by(year ) %>% 
  summarise(count = n(),
            dynast = sum(dyn),
            prop = dynast/count)%>%
  select(year, prop) %>% kable(digits =2, caption = "Dynasts among winners and runner-ups") %>% 
  kable_styling(full_width = F)
```
...
fit.1 <- felm(turnout~  dynast_winner+ reservation_SCST + margin_percentage+enop+criminal_1_2|0|0|state,data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))



fit.2 <- felm(turnout~  dynast_winner+ reservation_SCST + margin_percentage+enop+criminal_1_2|year|0|state, data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))

fit.3 <- felm(turnout~  dynast_winner+ reservation_SCST + margin_percentage+enop+criminal_1_2|state|0|state,data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))

fit.4<- felm(turnout~  dynast_winner+ reservation_SCST + margin_percentage+enop+criminal_1_2| state+year|0|state, data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))




fit.5<- felm(turnout~  dynast_winner+ reservation_SCST + margin_percentage+enop+criminal_1_2 | constituency_id+ state+year|0|state,data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))
...
fit.1 <- felm(turnout~  dynast_winner+ reservation_SCST + margin_percentage+enop+criminal_1_2+lagged_turnout|0|0|state,data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))



fit.2 <- felm(turnout~  dynast_winner+ reservation_SCST + margin_percentage+enop+criminal_1_2+lagged_turnout|year|0|state,data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))
fit.3 <- felm(turnout~  dynast_winner+ reservation_SCST + margin_percentage+enop+criminal_1_2+lagged_turnout|state|0|state,data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))

fit.4<- felm(turnout~  dynast_winner+ reservation_SCST + margin_percentage+enop+criminal_1_2+lagged_turnout| state+year|0|state,data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))




fit.5<- felm(turnout~  dynast_winner+ reservation_SCST + margin_percentage+enop+criminal_1_2+lagged_turnout|constituency_id+ state+year|0|state,data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))
...
fit.1 <- felm(turnout~  dynast_cand+ reservation_SCST + margin_percentage+enop+criminal_1_2|0|0|state,data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))



fit.2 <- felm(turnout~  dynast_cand+ reservation_SCST + margin_percentage+enop+criminal_1_2|year|0|state, data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))

fit.3 <- felm(turnout~  dynast_cand+ reservation_SCST + margin_percentage+enop+criminal_1_2|state|0|state,data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))

fit.4<- felm(turnout~  dynast_cand+ reservation_SCST + margin_percentage+enop+criminal_1_2| state+year|0|state, data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))




fit.5<- felm(turnout~  dynast_cand+ reservation_SCST + margin_percentage+enop+criminal_1_2 | constituency_id+ state+year|0|state,data = subset(df %>% distinct(year,constituency,.keep_all =TRUE)))
