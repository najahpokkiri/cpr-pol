...
# dim(dyn_mplads)
# 
#fwrite(dyn_mplads, "~/work/cpr/UP/up-dynasties/dyn_other_data/mplads-dyn.csv")
# 
# dyn_mplads %>%
#   mutate(log_wk = log(days/7)) %>% 
#   filter(days>0 & year ==2009) %>% 
#    infer::t_test(formula = log_wk ~ stable)
# t.test(dyn_mplads$stable, dyn_mplads$days)
# 
# mp_t <- dyn_mplads %>%
#    filter(days>0 & year ==2009 & !is.na(days)) %>% 
#   mutate(log_wk = log(days/7),
#          stabel_bin = ifelse(stable=="Stable Family",1,0)) 
# t.test(as.numeric(mp_t$days), mp_t$stabel_bin)
...
### Two type of comparisons{.tabset}


#### Family v/s Non-family


This table shows the average number of projects and the cost for the family types. First table is for both years and the second one is year by year break-up of the same.
...
dyn_mplads %>% group_by(dyn_cum_2_text) %>% summarise(count = n(), mean_peojects_pc = mean(ac_contr_n),mean_cost = mean(costnew), mean_weeks = mean(days/7)) %>% kable(digits = 0,col.names = c("Entity","Count of projects","Avg. # of projects in a PC","Avg. cost of a project", "Avg. # of weeks to sanction a project"), caption = "MPLADS 2004:2009 UP") %>% 
  kable_styling()
...
so we limit our dataset to 2009 to do this analysis


```{r}

dyn_mplads$stable <- ifelse(dyn_mplads$fam_type=="Stable", "Stable Family", "Non-Family")

dyn_mplads %>% filter(year==2009 & days>0)%>%  group_by(stable) %>% summarise(count = n(), mean_peojects_pc = mean(ac_contr_n),mean_cost = mean(log(costnew)), mean_weeks = mean(log(days/7))) %>% kable(digits = 0,col.names = c("Entity","Count of projects","Avg. # of projects in a PC","Avg. cost of a project", "Avg. # of weeks to sanction a project"),caption = "MPLADS 2009 - UP") %>% 
  kable_styling()
...
<!-- ```{r} -->
<!-- dyn_mplads%>% filter(days>0)%>% group_by(dyn_cum_2_text) %>% summarise(days = mean(days/7)) %>% kable(digits = 2, col.names = c("entity", "weeks")) %>%  -->
<!--   kable_styling(full_width = FALSE) -->
<!-- ``` -->


<!-- ```{r} -->
<!-- ggplot(dyn_mplads %>% filter(days>0), aes(dyn_cum_2_text, log(days/7)))+ -->
<!--   geom_boxplot()+ -->
<!--   labs(x= "entity", y = "weeks", title = "Weeks to sanction") -->
...
<!-- fam <- fread("~/work/cpr/UP/up-dynasties/dyn_other_data/fam_cvat_list.csv", header = TRUE) -->


<!-- mplads_fam <- inner_join(dyn_mplads_09, fam, by = "family_id") -->


<!-- mplads_fam%>% filter(days>0)%>% group_by(success_cat) %>% summarise(obs = n(),n_fam = n_distinct(family_id), mean_days = mean(days/7)) %>% kable(digits = 0, caption = "2009 MPLADS") -->
