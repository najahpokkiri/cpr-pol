...
fit.1 <- feglm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w+ log(total_assets)  + serious_crime + non_serious_crime+turncoat+election_type|constituency+year, family = binomial(link = "probit"), data = df)
...
etable(fit.1,fit.2,fit.3,cluster = "constituency",extraline=list("Caste control"=c("Yes",  "Yes", "Yes"))) %>%  slice(-(2:7)) %>% kable(caption = "Regression Results - AE GE 2009:19") %>% kable_styling(full_width = F)
