...
df <- tibble(dynast, caste_uc,caste_yadav,caste_non_yadav_obc , caste_dalit , caste_muslim ,minister_duration ,industry , school_college , land ,linkage_down,linkage_up, post_90 ,n_elections_w ,district, year)
...
fit.1 <- feglm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + post_90 +n_elections_w |district+year, family = binomial(link = "probit" ), data = df)


fit.2 <- feglm(linkage_up ~  caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + post_90+n_elections_w|district+year, family = binomial(link = "probit"), data = df)



 fit.3 <- feglm(linkage_down ~  caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + dynast+ post_90 +n_elections_w |district+year , family = binomial(link = "probit"), data = df)
 
 
  # fit.4 <- feglm( ~  caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + dynast+ post_90 +n_elections_w |district+year , family = binomial(link = "probit"), data = subset(df %>% filter(position ==1)))
  # 
  # 
 
 etable(fit.1, fit.2, fit.3,cluster = "district",extraline=list("Caste control"=c("Yes", "Yes", "Yes"))) %>%  slice(-(2:7)) %>% kable(caption = "Regression Results - AE All") %>% kable_styling(full_width = F)
 


  #  
  #  stargazer(fit.1, fit.2, fit.3,title="Regression Results - AE All ",type = "text",
...
fit.2 <- feglm(linkage~  caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + post_90 + n_elections_w| district+year , family = binomial(link = "probit"), data = df)

fit.3 <- feglm(linkage_down~  caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land +dynast+post_90 + n_elections_w | district +year, family = binomial(link = "probit"), data = df)

#  stargazer(fit.1, fit.2,title="Regression Results - AE-GE All ",type = "text",
# align=TRUE, dep.var.labels=c("Dynast","Linkage"), no.space=TRUE)
# # 


 
 etable(fit.1, fit.2, fit.3,cluster = "district",extraline=list("Caste control"=c("Yes", "Yes", "Yes"))) %>%  slice(-(2:7)) %>% kable(caption = "Regression Results - AE GE All") %>% kable_styling(full_width = F)
...
<!-- ```{r} -->
<!-- dyn_ae_12_17<- dyn_ae_ge %>% filter(year %in% c(2012,2017) & position ==1) -->
<!-- ``` -->
...
<!-- dynast <-dyn_ae_12_17$dyn_cum_2 -->


<!-- vote_share <- dyn_ae_12_17$vote_share_percentage -->


<!-- industry <- dyn_ae_12_17$indus_count %>% replace_na(0) -->

<!-- industry_rent_thick <- dyn_ae_12_17$indus_rt %>% replace_na(0) -->

<!-- industry_non_rent_thick <- dyn_ae_12_17$indus_nrt %>% replace_na(0) -->

<!-- caste_uc <- ifelse(dyn_ae_12_17$caste_groups == "Upper Caste", 1,0) -->


<!-- caste_yadav <- ifelse(dyn_ae_12_17$caste_groups == "Yadav", 1,0) -->

<!-- caste_muslim <- ifelse(dyn_ae_12_17$caste_groups == "Muslim", 1,0) -->

<!-- caste_dalit <- ifelse(dyn_ae_12_17$caste_groups == "Dalit", 1,0) -->

<!-- caste_non_yadav_obc <- ifelse(dyn_ae_12_17$caste_groups == "Non-Yadav OBC", 1,0) -->
...
<!-- minister <-ifelse(dyn_ae_12_17$min_dur_yr>0,1,0)%>% replace_na(0)  -->


<!-- n_elections_w <- dyn_ae_12_17$n_el_c_ae -->


<!-- school <- dyn_ae_12_17$school%>% replace_na(0) -->

<!-- college <- dyn_ae_12_17$college%>% replace_na(0) -->
...
<!-- land <- as.numeric(dyn_ae_12_17$land) %>% replace_na(0) -->

<!-- total_assets <- dyn_ae_12_17$total_assets -->

<!-- assets_above_median <- ifelse(dyn_ae_12_17$total_assets>median(dyn_ae_12_17$total_assets),1,0) -->

<!-- serious_crime <- ifelse(dyn_ae_12_17$serious_crime>0, TRUE, FALSE) -->

<!-- winner <- ifelse(dyn_ae_12_17$position==1, 1,0) -->

<!-- association <- ifelse(dyn_ae_12_17$association_inv>0,1,0)%>% replace_na(0) -->

<!-- organisation <- ifelse(dyn_ae_12_17$org_inv>0,1,0) %>% replace_na(0) -->

<!-- politics <- ifelse(dyn_ae_12_17$poitical_inv>0,1,0) %>% replace_na(0)  -->


<!-- linkage <- ifelse(dyn_ae_12_17$level_cum %in% c("AE", "GE"),0,1) -->

<!-- linkage_down <- ifelse(dyn_ae_12_17$level_cum %in% c("AE-LB", "GE-AE-LB","GE-LB"),1,0) -->

<!-- district <- dyn_ae_12_17$district_name -->

<!-- year <- dyn_ae_12_17$year -->

<!-- dynast_winner <- ifelse(dyn_ae_12_17$dyn_cum_2==1,1,0) -->

<!-- dyn_ae_12_17$incumebnt <-  -->

<!-- df <- tibble(dynast , caste_uc, caste_yadav,caste_non_yadav_obc , caste_dalit , caste_muslim ,minister_duration ,industry , school_college , land ,linkage_down, n_elections_w,  total_assets  , serious_crime , turncoat, winner, vote_share,district, year, dyn_ae_12_17$enop, dyn_ae_12_17$turnout_percentage, dyn_ae_12_17$margin_percentage, dyn_ae_12_17$constituency_type, dynast_winner,dyn_ae_12_17$incumbent -->
<!--              ) -->
<!-- ``` -->


<!-- ```{r} -->
<!-- fit.2 <- feglm(dynast_winner~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration+industry + school_college + land + n_elections_w + log(total_assets) + serious_crime + turncoat + linkage_down+`dyn_ae_12_17$enop`+ `dyn_ae_12_17$turnout_percentage`+ `dyn_ae_12_17$margin_percentage`+`dyn_ae_12_17$incumbent`+ `dyn_ae_12_17$constituency_type`|district+year , family = binomial(link = "probit"), data = df) -->


<!-- etable(fit.2,extraline=list("Caste control"=c("Yes")),drop= c("caste_uc","caste_yadav","caste_non_yadav_obc", "caste_muslim", "caste_dalit")) %>% kable() -->
...
df <- tibble(dynast , caste_uc, caste_yadav,caste_non_yadav_obc , caste_dalit , caste_muslim ,minister_duration ,industry , school_college , land ,linkage_down, n_elections_w,  total_assets  , serious_crime , turncoat, winner, vote_share,district, year
             )
```


```{r  results = "asis"}
fit.1 <- feglm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w+ log(total_assets)  + serious_crime + turncoat |district+year, family = binomial(link = "probit"), data= df)


#summary(fit.2)

fit.2 <- feglm(winner ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w + log(total_assets) + serious_crime + turncoat + dynast + linkage_down|district+year , family = binomial(link = "probit"), data = df)

fit.3 <- felm(vote_share ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w + log(total_assets) + serious_crime + turncoat + dynast + linkage_down|district+year|0|district, data = df)

etable(fit.1,fit.2,cluster = "district",extraline=list("Caste control"=c("Yes",  "Yes"))) %>%  slice(-(2:7)) %>% kable(caption = "Regression Results - AE 2012-17") %>% kable_styling(full_width = F)
```
...
```{r results = "asis"}
fit.1 <- feglm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w+ log(total_assets) + serious_crime + turncoat|pc+year, family = binomial(link = "probit"), data= df)




fit.2 <- feglm(winner ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w + log(total_assets) + serious_crime + turncoat + dynast + linkage_down|pc+year, family = binomial(link = "probit"), data = df)

fit.3 <- felm(vote_share ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w + log(total_assets)  + serious_crime + turncoat + dynast+ linkage_down|pc+year|0|pc, data = df)



#summary(fit.3)

etable(fit.1,fit.2,cluster = "pc",extraline=list("Caste control"=c("Yes",  "Yes"))) %>%  slice(-(2:7)) %>% kable(caption = "Regression Results - GE 2009-19") %>% kable_styling(full_width = F)
...
fit.1 <- feglm(dynast ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w+ log(total_assets)  + serious_crime + non_serious_crime+turncoat|constituency+year, family = binomial(link = "probit"), data = df)




fit.2 <- feglm(winner ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w + log(total_assets) + serious_crime + turncoat + dynast + linkage_down+election_type|constituency+year, family = binomial(link = "probit"), data = df)



fit.3 <- feglm(linkage ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w + log(total_assets) + serious_crime + turncoat+election_type |constituency+year , family = binomial(link = "probit"), data= df)

fit.4 <- felm(vote_share ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry + school_college + land + n_elections_w + log(total_assets)  + serious_crime + turncoat + dynast + linkage_down+election_type|constituency+year|0|year)

#summary(fit.1)


etable(fit.1,fit.2,fit.3,cluster = "constituency",extraline=list("Caste control"=c("Yes",  "Yes", "Yes"))) %>%  slice(-(2:7)) %>% kable(caption = "Regression Results - AE 2012-17") %>% kable_styling(full_width = F)

stargazer( fit.4,title="Regression Results - AE 2012:17 ",type = "html",
...
# align=TRUE, dep.var.labels=c("Dynast","Winner", "Linkage","Vote Share Percentage"), no.space=TRUE,add.lines=list(c('Constituency fixed effects', 'Yes' )))
