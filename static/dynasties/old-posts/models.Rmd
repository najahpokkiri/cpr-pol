...
fit.3 <- feglm(won_elections~  caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim+linkage +minister_duration +industry +  land + post_90, data = df)
 ...
 myDict = c("industry" = "Economic diversification", "school_college" = "Educational ownership", "land" = "Land ownership", "n_elections_w" = "Number of elections won", "linkage" = "Political diversification", "minister_duration" = "Minister duration", "dynast" = "Family")
  ...
  order= c("dynast","Political diversification","linkage" , "minister_duration","industry" , "land" , "n_elections_w", "won_elections" )
...
etable(fit.3,extraline=list("Caste control"=c("Yes")),drop = drop, order = order ,dict = myDict)  %>% kable(caption = "Elections won - AE All") %>% kable_styling(full_width = F)
```
...
land<- dyn_ae_ge_uniq$land_decile
...
etable(fit.3,extraline=list("Caste control"=c("Yes")),drop = drop, order = order ,dict = myDict)  %>% kable(caption = "Elections won - AE and GE All") %>% kable_styling(full_width = F)
```
...
land<- dyn_ae_12_17_uniq$land_decile
...
land<- dyn_ge_09_19_uniq$land_decile
...
land<- dyn_ae_ge_09_19_uniq$land_decile
...
df <- tibble(winner ,   incumbent,  turncoat ,caste_UC_IC, female , terms , dynast , assets  , serious_crime, education,age, state,year,enop, reservation_SCST,recontest, constituency, party, vote_share_pc)
# 
...
cbind(stat, vals) %>% kable(digits = 2,  caption = "Summary of GE 5% data") %>% kable_styling(full_width = F)
...
fit_year <- feglm(winner ~   incumbent+  turncoat +caste_UC_IC+ female + terms + dynast + log(assets)  + serious_crime+ education+age | year, family = binomial(link = "probit" ), data = df )


fit_state <- feglm(winner ~   incumbent+  turncoat +caste_UC_IC+ female + terms + dynast + log(assets)  + serious_crime+ education+age | state+year,  family = binomial(link = "probit" ),data = df )

fit_ac <- feglm(winner ~   incumbent+  turncoat +caste_UC_IC+ female + terms + dynast + log(assets)  + serious_crime+ education+age | state+year+constituency, family = binomial(link = "probit" ), data = df )


fit_party<- feglm(winner ~   incumbent+  turncoat +caste_UC_IC+ female+ terms + dynast + log(assets)  + serious_crime+ education+age | state+year+constituency+party,  family = binomial(link = "probit" ),data = df )

order = c("winner" , "vote_share_pc", "incumbent",  "turncoat" , "terms", "dynast" , "assets", "serious_crime", "education", "age","caste_UC_IC", "female")
...
fit_year <- felm(vote_share_pc ~   incumbent+  turncoat +caste_UC_IC+ female+  terms + dynast + log(assets)  + serious_crime+ education+age | year, data = subset(df %>% filter(assets>0))  )




fit_state <- felm(vote_share_pc ~   incumbent+  turncoat +caste_UC_IC+ female+  terms + dynast + log(assets)  + serious_crime+ education+age | year+state|0|state, data = subset(df %>% filter(assets>0))  )



fit_ac<- felm(vote_share_pc ~   incumbent+  turncoat +caste_UC_IC+ female+ terms + dynast + log(assets)  + serious_crime+ education+age | year+state+constituency|0|constituency+state, data = subset(df %>% filter(assets>0)) )


fit_party <- felm(vote_share_pc ~   incumbent+  turncoat +caste_UC_IC+ female + terms + dynast + log(assets)  + serious_crime+ education+age| year+state+constituency + party|0|constituency+state,data = subset(df %>% filter(assets>0)) )

# etable(fit_year,fit_state,fit_ac, fit_party, cluster = c("state","constituency"), order = order) %>% kable(caption = "UP vote share")


list <- list(c("Fixed effects","Year" , "Year + State", "Year + State + Constiuency", "Year+State+Constiuency+Party"), c("SE Clustered", "None","State","Constituency+ State","Constituency+ State" ))



stargazer( fit_year,fit_state,fit_ac, fit_party,title="Vote share - India ",type = "html",
...
fit_ac <- feglm(winner ~   incumbent+  turncoat +caste_UC_IC+ female + terms + dynast + log(assets)  + serious_crime+ education+age |year+constituency, family = binomial(link = "probit" ), data = df )


fit_party<- feglm(winner ~   incumbent+  turncoat +caste_UC_IC+ female+ terms + dynast + log(assets)  + serious_crime+ education+age | state+constituency+party,  family = binomial(link = "probit" ),data = df )

order = c("winner" , "vote_share_pc", "incumbent",  "turncoat" , "terms", "dynast" , "assets", "serious_crime", "education", "age","caste_UC_IC", "female")


etable(fit_year, fit_ac, fit_party, cluster = c("constituency"), order = order) %>% kable()%>% kable_styling(full_width = F)
```

#### Vote share

```{r results="asis"}
fit_year <- felm(vote_share_pc ~   incumbent+  turncoat +caste_UC_IC+ female + terms + dynast + log(assets)  + serious_crime+ education+age | year, data = df )


fit_ac <- felm(vote_share_pc ~   incumbent+  turncoat +caste_UC_IC+ female + terms + dynast + log(assets)  + serious_crime+ education+age | year+constituency|0|constituency, data = df )

fit_party <- felm(vote_share_pc ~   incumbent+  turncoat +caste_UC_IC+ female + terms + dynast + log(assets)  + serious_crime+ education+age |  year+constituency+party|0|constituency, data = df )
# 
# etable(fit_year,fit_ac, fit_party, cluster = c("constituency"), order = order) %>% kable(caption = "UP vote share")

list <- list(c("Fixed effects","Year" ,  "Year + Constiuency", "Year+Constiuency+Party"), c("SE Clustered", "None","Constituency","Constituency" ))



stargazer( fit_year,fit_ac, fit_party,title="Vote share - UP ",type = "html",
align=TRUE, no.space=TRUE, order = order,add.lines=list)
