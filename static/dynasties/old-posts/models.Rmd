...
minister_duration <- ntile(minister_duration,5)
...
n_elections_w <- ntile(n_elections_w,5)

#elections_w_2<-  ifelse(elections_w_2>1,1,0)

won_elections <- dyn_ae_uniq$n_el_w
...
industry <- ntile(industry, 5)


  dyn_ae_uniq$land <-  as.numeric(dyn_ae_uniq$land) 
  
  land <- ifelse(dyn_ae_uniq$land>1000,1000,dyn_ae_uniq$land) %>% replace_na(0)
...
```{r  results= "asis"}
df <- tibble(dynast, caste_uc,caste_yadav,caste_non_yadav_obc , caste_dalit , caste_muslim ,minister_duration ,industry ,  land ,linkage,linkage_down,linkage_up, post_90 ,n_elections_w ,district, year, won_elections)
...
fit.3 <- feglm(won_elections~  caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim+linkage +minister_duration +industry +  land + post_90, data = df)

 
 myDict = c("industry" = "Economic diversification", "school_college" = "Educational ownership", "land" = "Land ownership", "n_elections_w" = "Number of elections won", "linkage" = "Political diversification", "minister_duration" = "Minister duration", "dynast" = "Family","caste_uc" = "Caste_uc", "caste_yadav" = "Caste_yadav","caste_non_yadav_obc" = "Caste_non_yadav_obc", "caste_dalit"=  "Caste_dalit", "caste_muslim"="Caste_muslim")
 
 
   order= c("Economic diversification", "Educational ownership",  "Land ownership",  "Number of elections won", "Political diversification", "Minister duration", "Family", "Caste_uc",  "Caste_yadav", "Caste_non_yadav_obc",   "Caste_dalit","Caste_muslim")
 
  # order= c("dynast","Political diversification","linkage" , "minister_duration","industry" , "land" , "n_elections_w", "won_elections" ,"caste_uc", "caste_yadav","caste_non_yadav_obc", "caste_dalit" , "caste_muslim")
  
  drop <- c("caste_uc", "caste_yadav","caste_non_yadav_obc", "caste_dalit", "caste_muslim","(Intercept)")
...
```{r  results= "asis"}
etable(fit.1,cluster = "district",extraline=list("Caste control"=c("Yes")),dict = myDict , order = order )  %>% kable(caption = "Regression Results - AE All") %>% kable_styling(full_width = F)
```
...
etable(fit.2,cluster = "district",extraline=list("Caste control"=c("Yes")), order = order ,dict = myDict)  %>% kable(caption = "Regression Results - AE All") %>% kable_styling(full_width = F)
```

```{r  results= "asis"}

etable(fit.3,extraline=list("Caste control"=c("Yes")), order = order ,dict = myDict)  %>% kable(caption = "Elections won - AE All") %>% kable_styling(full_width = F)
```
...
won_elections <- dyn_ae_ge_uniq$n_el_w
...
dyn_ae_ge_uniq$land <-  as.numeric(dyn_ae_ge_uniq$land) 

land <- ifelse(dyn_ae_ge_uniq$land>1000,1000,dyn_ae_ge_uniq$land) %>% replace_na(0)
...
#  dyn_ae_ge_uniq$land <- as.numeric(dyn_ae_ge_uniq$land) %>% replace_na(0)
# 
# dyn_ae_ge_uniq %>% mutate(land_cat = cut(as.numeric(land), breaks = c(-Inf,0,100,500,1000,Inf), labels = c("0","0-100","100-500","500-1000","1000+"))) %>% group_by(land_cat) %>% summarise(count = n(), pc = count/length(dyn_ae_ge_uniq$land)) %>% kable(digits = 2)
# 
# 
# 
# 
# 
# dyn_ae_ge_uniq %>% group_by(land_decile) %>% summarise(count = n(), mean = mean(land))
# 
# 
# minister_quratile <- ntile(minister, 4)
# 
# table(minister_quratile)
# 
# table(minister)
...
# dyn_ae_ge %>% filter(dyn_tot_2==1 & relative_id!=1) %>% group_by(relative_id) %>% summarise(count = n())
# 
# 
# dyn_ae_ge %>% filter(dyn_tot_2==1 & relative_id!=1) %>% ungroup() %>% tally()

```

```{r results = "asis"}
df  <- tibble(dynast , caste_uc, caste_yadav,caste_non_yadav_obc , caste_dalit , caste_muslim ,minister_duration ,industry , school_college , land ,linkage_down, post_90 , n_elections_w,district,linkage, year, won_elections)
...
fit.3 <- feglm(won_elections ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim+linkage_down +minister_duration +industry + land + post_90  , data = df)
 ...
 # 
 #  order= c("Family","Political diversification" ,"linkage_down", "minister_duration","industry" ,"school_college", "land" , "n_elections_w", "won_elections"  )
 #  
 #  drop <- c("caste_uc", "caste_yadav","caste_non_yadav_obc", "caste_dalit", "caste_muslim","(Intercept)")
```
...
```{r results = "asis"}
etable(fit.1,cluster = "district",extraline=list("Caste control"=c("Yes")), order = order ,dict = myDict)  %>% kable(caption = "Regression Results - AE All") %>% kable_styling(full_width = F)
```
...
etable(fit.3,extraline=list("Caste control"=c("Yes")), order = order ,dict = myDict)  %>% kable(caption = "Elections won - AE and GE All") %>% kable_styling(full_width = F)
```
...
dyn_ae_12_17_uniq$land <-  as.numeric(dyn_ae_12_17_uniq$land) 

land <- ifelse(dyn_ae_12_17_uniq$land>1000,1000,dyn_ae_12_17_uniq$land) %>% replace_na(0)

land<- ntile(land,5)

total_assets <- ntile(total_assets,5)
...
fit.3 <- felm(vote_share ~ caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim +minister_duration +industry +  land + n_elections_w + log(total_assets) + serious_crime + turncoat + dynast + linkage|district+year|0|district, data = df)
 ...
 myDict = c("industry" = "Economic diversification", "school_college" = "Educational ownership", "land" = "Land ownership", "n_elections_w" = "Number of elections won", "linkage" = "Political diversification", "minister_duration" = "Minister duration", "dynast" = "Family","serious_crimeTRUE" = "Serious crime","caste_uc" = "Caste_uc", "caste_yadav" = "Caste_yadav","caste_non_yadav_obc" = "Caste_non_yadav_obc", "caste_dalit"=  "Caste_dalit", "caste_muslim"="Caste_muslim")
 
 
   order= c("Economic diversification", "Educational ownership",  "Land ownership",  "Number of elections won", "Political diversification", "Minister duration", "Family","log(total_assets)", "Serious crime","Caste_uc",  "Caste_yadav", "Caste_non_yadav_obc",   "Caste_dalit","Caste_muslim")
...
```{r  results = "asis"}
etable(fit.1,cluster = "district",extraline=list("Caste control"=c("Yes")), order = order ,dict = myDict)  %>% kable(caption = "Regression Results - AE 2012-17") %>% kable_styling(full_width = F)
```
...
etable( fit.2,cluster = "district",extraline=list("Caste control"=c("Yes")), order = order ,dict = myDict)  %>% kable(caption = "Regression Results - AE 2012-17") %>% kable_styling(full_width = F)
```
...
dyn_ge_09_19_uniq$land <-  as.numeric(dyn_ge_09_19_uniq$land) 

land <- ifelse(dyn_ge_09_19_uniq$land>1000,1000,dyn_ge_09_19_uniq$land) %>% replace_na(0)
...
```{r results = "asis"}
etable(fit.1,cluster = "pc",extraline=list("Caste control"=c("Yes")), order = order ,dict = myDict)  %>% kable(caption = "Regression Results - AE All") %>% kable_styling(full_width = F)
```
...
etable( fit.2,cluster = "pc",extraline=list("Caste control"=c("Yes")), order = order ,dict = myDict)  %>% kable(caption = "Regression Results - AE All") %>% kable_styling(full_width = F)
```
...
land <- ifelse(dyn_ae_ge_09_19_uniq$land>1000,1000,dyn_ae_ge_09_19_uniq$land) %>% replace_na(0)
...
```{r results="asis"}
etable(fit.1,cluster = "constituency",extraline=list("Caste control"=c("Yes")), order = order ,dict = myDict)  %>% kable(caption = "Regression Results - AE All") %>% kable_styling(full_width = F)
```

---
```{r results="asis"}

etable( fit.2, cluster = "constituency",extraline=list("Caste control"=c("Yes")), order = order ,dict = myDict)  %>% kable(caption = "Regression Results - AE All") %>% kable_styling(full_width = F)
```

---

```{r}
etable(fit.3,cluster = "constituency",extraline=list("Caste control"=c("Yes")), order = order ,dict = myDict)  %>% kable(caption = "Regression Results - AE All") %>% kable_styling(full_width = F)
```
...
align=TRUE, dep.var.labels=c( "Vote Share Percentage"), no.space=TRUE, order = order)
