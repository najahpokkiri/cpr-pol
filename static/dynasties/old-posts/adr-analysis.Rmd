...
dyn_ae <- dyn_ae %>% select(1:1, 3:4,8:13)
...
dyn_ae_adr <-dyn_ae_adr %>% group_by(year) %>% mutate(total_assets =  DescTools::Winsorize(as.numeric(total_assets), na.rm = TRUE,probs = c(.01,.99)))#%>% filter(total_assets<1000000000)

assets_caste <- ggplot(dyn_ae_adr , aes(caste_groups, total_assets))+
  geom_boxplot()+
  facet_wrap(~year)+
  labs(x = "", y = "Total assets", x = "",fill = "",title = "Asset") +
    scale_y_log10(
...
df_stack <- dyn_ae_adr %>% group_by(year,caste_groups) %>% summarise(total = n(), 
  ...
  criminal_caste_stacked <- ggplot(df_stack,aes(variable, value, fill = caste_groups,label = ifelse(value>.05,round(value,2),"")))+
   geom_bar(stat="identity", position = "stack")+
    geom_text(size = 2.5, position = position_stack(vjust = 0.5))+ 
    facet_wrap(~year)+
     scale_x_discrete(labels = c("All candidates","Winners","Candidates \n w/ 1 case","Candidate \n w/ 2+ case"))+
    scale_fill_grey()+
  #colorspace::scale_fill_discrete_sequential(palette= "grays",rev= FALSE)+
    theme_minimal()+
   
      
    labs(x = "", y = "Proportion", fill = "", title = "Criminality") +
...
#   
ggpubr::ggarrange(criminal_caste_stacked, assets_caste)
#   
# 
 cowplot::plot_grid(criminal_caste_stacked, assets_caste,align = "hv")
# 
# library(patchwork)
 p3_6 <- assets_caste + criminal_caste_stacked 
 
   
ggsave("D:/cpr/thesis/figures/fig_3_6.png", p3_6,width = 10, height =6, dpi = 300, units ="in" )
