...
```{r}
 cowplot::plot_grid(ggplot(ass_rates, aes(tot_change, fill = success_cat))+
  geom_density(alpha = .5)+
  theme_minimal()+
  labs(title = "Percentage growth of total assets ", subtitle = "AE 2012-17")+
  theme(legend.position = "none"),

 ggplot(ass_rates, aes(mov_change, fill = success_cat))+
  geom_density(alpha = .5)+
  theme_minimal()+
  labs(title = "Percentage growth of movable assets ", subtitle = "AE 2012-17")+
  theme(legend.position = "none"),

 ggplot(ass_rates, aes(immov_change, fill = success_cat))+
  geom_density(alpha = .5)+
  theme_minimal()+
  labs(title = "Percentage growth of immovable assets ", subtitle = "AE 2012-17"))
...
```{r}
 ...
 cowplot::plot_grid(ggplot(ass_rates %>% filter(between(tot_change, quantile(tot_change, .025), quantile(tot_change,.975))), aes(tot_change, fill = factor(dyn_cum_2)))+
  geom_density(alpha = .5)+
  theme_minimal()+
  labs(title = "Percentage growth of total assets ", subtitle = "GE 2009:19")+
  theme(legend.position = "none"),


 ggplot(ass_rates %>% filter(between(mov_change, quantile(mov_change, .025), quantile(mov_change,.975))), aes(mov_change, fill = factor(dyn_cum_2)))+

  geom_density(alpha = .5)+
  theme_minimal()+
  labs(title = "Percentage growth of movable assets ", subtitle = "GE 2009:19")+
  theme(legend.position = "none")
 ,
 
 
  ggplot(ass_rates %>% filter(between(immov_change, quantile(immov_change, .025), quantile(immov_change,.975))), aes(immov_change, fill = factor(dyn_cum_2)))+
  geom_density(alpha = .5)+
      scale_fill_discrete(labels = c("Non-family", "Family"))+
  theme_minimal()+
  labs(title = "Percentage immovable of total assets ", subtitle = "AE 2012-17", fill = ""))
```
...
```{r}
 ...
 cowplot::plot_grid(ggplot(ass_rates %>% filter(between(tot_change, quantile(tot_change, .025), quantile(tot_change,.975))), aes(tot_change, fill = factor(dyn_cum_2)))+
  geom_density(alpha = .5)+
  theme_minimal()+
  labs(title = "Percentage growth of total assets ", subtitle = "GE 2014-19")+
  theme(legend.position = "none"),
 ...
 ggplot(ass_rates %>% filter(between(mov_change, quantile(mov_change, .025), quantile(mov_change,.975))), aes(mov_change, fill = factor(dyn_cum_2)))+
  ...
  geom_density(alpha = .5)+
  theme_minimal()+
  labs(title = "Percentage growth of movable assets ", subtitle = "GE 2014-19")+
  theme(legend.position = "none")
 ,
  ...
  geom_density(alpha = .5)+
      scale_fill_discrete(labels = c("Non-family", "Family"))+
  theme_minimal()+
  labs(title = "Percentage immovable of total assets ",subtitle = "GE 2014-19", fill = ""))
```
