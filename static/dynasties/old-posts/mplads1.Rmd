...
**Average time approval for the project types**
...
<!--   ggplot(mp_car_dyn %>% filter(app.time>0), aes(reorder(clean.sector, -app.time, FUN = median),log(app.time+1)))+ -->
<!--   geom_boxplot()+ -->
<!--     geom_jitter(alpha = .3)+ -->
...
```{r}
  ...
  ggplot(mp_car_dyn %>% filter(app.time>0 & clean.sector 
                               %in% c("education", "electricty", "other",
                                      "roads", "water")), aes(stable,log(app.time+1)))+

  geom_boxplot()+
  geom_jitter(alpha = .3)+
   stat_summary(fun=mean, colour="darkred", geom="point", 
               shape=15, size=1.5, show.legend=FALSE) + 
  facet_grid(~clean.sector)+
labs(title = "Average weeks to approval",
       y = "Log of weeks",
       x = "Project Type")+
  theme_light()+
...
```{r}
  ...
  ggplot(mp_car_dyn %>% filter(app.time>0 & clean.sector 
                               %in% c("education", "electricty", "other",
                                      "roads", "water")), aes(stable,log(amt_sanc+1)))+
...
mplads_all <- mplads.eval %>%rename_all(tolower) %>% mutate(state = tolower(state)) %>% 
  ...
  dyn_ge  <- read_csv("~/work/cpr/UP/up-dynasties/dyn_other_data/dyn_5pc_8919.csv") %>% 
    ...
    select(state_name, constituency_no, dynast = dyn, year) %>% 
