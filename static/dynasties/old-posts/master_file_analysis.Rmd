...
dyn_non_fam_w <- dyn_ae_ge %>% filter(dyn_tot_2==0 &n_elections_won >=1)%>% group_by(family_id) %>% summarise(max_year = max(year), min_year = min(year)) %>% mutate(life_span = max_year-min_year) 
