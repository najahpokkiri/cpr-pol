...
# dyn <- dyn %>% group_by(family_id) %>% 
#   mutate(level1 = paste0(unique(election_level), collapse = '-'))
# 
# 
# dyn <- dyn %>% mutate(level2 = case_when(level1 %in% c("AE-GE-LB","GE-AE-LB","GE-LB-AE","LB-AE-GE",  "AE-LB-GE" ) ~"GE-AE-LB",
...
dyn_ae_ge <- dyn_ae_ge %>% group_by(family_id) %>% mutate(max_year = max(year), min_year = min(year),life_span = max_year-min_year)
...
dyn_ae_ge <- dyn_ae_ge %>% group_by(fam_rel_uniq_id) %>% mutate(n_elections_contested = n_distinct(election_id))%>% ungroup(fam_rel_uniq_id)
...
pc_shp_up <- pc_shp %>% filter(ST_NAME == "UTTAR PRADESH")




fam_pc <- dyn_ae_ge %>% filter(dyn_cum_2==1) %>%arrange(-year) %>%distinct(family_id, .keep_all = TRUE) %>%  group_by(pc_no) %>% summarise(n_fam = n())



pc_shp_fam <- left_join(pc_shp_up, fam_pc, by = c("PC_CODE" = "pc_no"))

pc_shp_fam$n_fam <- ifelse(is.na(pc_shp_fam$n_fam),0,pc_shp_fam$n_fam)


library(cowplot)

p3_1 <- ggplot(pc_shp_fam)+
  geom_sf(aes(fill = n_fam))+
  scale_fill_gradientn(colours = c("white", "grey50", "grey3"), values = c(0,0.1,1))+
  theme_void()+
  labs( fill = "Number of families",
       caption= "Source: Uttar Pradesh Political Families data set")
  

p3_1

ggsave("D:/cpr/thesis/figures/fig_3_1.png", p3_1,width = 5, height =3, dpi = 300, units ="in" ,)
...
dyn_all_uniq <- dyn%>% group_by(family_id) %>% filter(year == max(year)) %>% arrange(-year) %>% distinct(family_id, .keep_all = TRUE)

dyn_all_uniq_fam <- dyn_all_uniq %>% filter(dyn_tot_2 ==1)

dyn_all_uniq_fam %>% group_by(level) %>% summarise(count = n()) %>% arrange(-count)%>% kable(caption = "Linkages of families(Category 2)")%>%kable_styling(full_width = F)



#dyn_all_uniq_fam %>% group_by(level) %>% summarise(count = n()) %>% arrange(-count)
...
dyn_all_uniq_fam <- dyn_all_uniq %>% filter(dyn_tot_4 ==1)

dyn_all_uniq_fam %>% group_by(level) %>% summarise(count = n()) %>% arrange(-count)%>% kable(caption = "Linkages of families(Category 4)")%>%kable_styling(full_width = F)
...
p1_3 <- ggplot(ae_dyn, aes(x = factor(year), y=prop, group = 1))+
  
    geom_point(size =2)+
  geom_line(size = 1)+
  theme_bw()+

  labs( x = "Year", y = "Proportion",caption= "Source: Uttar Pradesh Political Families data set")+
...
ggsave("D:/cpr/thesis/figures/fig_1_3.png", p1_3,width = 6, height =3, dpi = 300, units ="in" )
...
dyn_ae_ge %>%group_by(family_id) %>% filter(dyn_cum_2==1) %>% arrange(year) %>% filter(year == min(year)) %>% distinct(family_id, .keep_all = TRUE) %>% select(family_id, year) %>% group_by(year) %>% summarise(count = n()) %>% mutate(cumsum= cumsum(count), pc = cumsum/322*100) %>% 
  ...
  # )
  # 
  #dyn_ae_ge %>%group_by(family_id) %>% filter(dyn_cum_2==1) %>% arrange(year) %>% filter(year == min(year)) %>% distinct(family_id, .keep_all = TRUE) %>% select(family_id, year) 
...
dyn_ae_ge$year_cat <- factor(dyn_ae_ge$year_cat , levels = c("77-88", "89-99", "00-08", "09-19"))

dyn_ae_ge$caste_groups <- factor(dyn_ae_ge$caste_groups, levels = c("Upper Caste","Yadav","Non-Yadav OBC","Dalit" , "Muslim"  ,"Others" ))

dyn_ae_ge %>%group_by(family_id) %>% filter(dyn_cum_2==1) %>% arrange(year) %>% filter(year == min(year)) %>% distinct(family_id, .keep_all = TRUE) %>% select(family_id, year_cat, caste_groups) %>%  group_by(year_cat, caste_groups)%>% summarise(count = n()) %>%arrange(-year_cat) %>% group_by(caste_groups) %>% mutate(cumsum = cumsum(count)) %>% group_by(year_cat) %>% mutate(sum = sum(cumsum) , pc = cumsum/sum*100) %>%
  ggplot(aes(year_cat, pc, fill = caste_groups,label = round(pc,0)))+
  geom_bar(position = "stack", stat = "identity")+
    ...
    theme_minimal()+
labs(title = "Cumulative caste composition of families  ",x = "", y = "", fill = "Caste Groups")+
    theme(plot.margin = unit(c(.5,.5,.5,.5), "cm"))+
...
```{r}
p1_4 <- dyn_ae_ge %>% filter(position ==1& year != 1974) %>% group_by(year, election_type, contest_type) %>% summarise(count = n()) %>% group_by(election_type, year) %>% mutate(sum = sum(count), prop = count/sum) %>%  filter(contest_type== "non-fam v/s non-fam") %>% select(election_type, year,contest_type, prop) %>% mutate(dyn_prop = (1-prop)*100) %>% 
  ggplot(aes(year, dyn_prop))+
  geom_line(aes(linetype=election_type))+
  geom_point()+
  #viridis::scale_color_viridis(discrete = TRUE)+

  ylim(0,60)+
   labs(#title = "Seats contested by dynasts in each elections", 
     y = "Percentage", x ="Year",
        caption= "Source: Uttar Pradesh Political Families data set")+
  theme_bw()+
  theme(legend.position = "bottom",
...
ggsave("D:/cpr/thesis/figures/fig_1_4.png", p1_4,width = 6, height =3.5, dpi = 300, units ="in" )
```
...
```{r}
dyn_ae_ge <- dyn_ae_ge %>% group_by(uniq_id) %>% mutate(n_el_w_ind = length(election_id[position==1]), n_el_c_ind  =n() ) %>% ungroup(uniq_id)

count_tab <- dyn_ae_ge %>% group_by(uniq_id) %>%filter(dyn_tot_2 != 1) %>%   summarise(n_el_w_ind = length(election_id[position==1]), n_el_c_ind  =n() ) %>% distinct(uniq_id, .keep_all = TRUE)

table(count_tab$n_el_c_ind)

table(count_tab$n_el_w_ind)
```
...
dyn_ae_ge <- dyn_ae_ge %>% group_by(uniq_id) %>% mutate(n_el_w_ind = length(election_id[position==1])) %>% ungroup(uniq_id)
...
dyn_ae_ge_fam <- dyn_ae_ge_fam %>% group_by(family_id) %>% mutate(n_el_w_fam =n_distinct(election_id[position==1 & dyn_cum_2==1]), n_el_c_fam = n_distinct(election_id[dyn_cum_2==1]))
...
ggplot(dyn_ae_ge_fam_uniq, aes(n_el_w_fam))+
...
# 
#dyn_ae_ge_fam_uniq %>% select(n_el_w_fam, n_el_c_fam) %>% filter(n_el_c_fam==0)
# 
#
```

#### Both - contested and won

```{r}
n_val <- dyn_ae_ge_fam_uniq %>% select(n_el_w_fam, n_el_c_fam) %>% pivot_longer(cols = 2:3)
# 
ggplot(n_val, aes(value, fill =name ))+
 geom_density(alpha = .5)+
  theme_minimal()+
  scale_fill_discrete(labels = c("Contested", "Won"))+
  labs(fill = "", title= "Families contested and won")
```
...
<!--   dyn_ae_ge %>% select(family_id,dyn_tot_2,n_el_w, n_el_c) %>% distinct(family_id, .keep_all = TRUE) %>% select(family_id, dyn_tot_2, n_el_c, n_el_w) %>% select(1:3) %>%  -->
<!-- ggplot( aes(n_el_c, fill =factor(dyn_tot_2) ))+ -->
<!--  geom_density(alpha = .5) -->


<!--     dyn_ae_ge %>% select(family_id,dyn_tot_2,n_el_w, n_el_c) %>% filter(dyn_tot_2 ==0)%>% distinct(family_id, .keep_all = TRUE) %>% select(family_id, dyn_tot_2,  n_el_w,n_el_c) %>% select(1:3) %>% ungroup() %>%   -->
<!-- ggplot( aes(n_el_w ))+ -->
<!--       geom_density() -->
<!--       geom_histogram() -->
...
<!--  dyn_ae_ge %>% select(family_id,dyn_tot_2,n_el_w, n_el_c) %>% distinct(family_id, .keep_all = TRUE) %>% select(family_id, dyn_tot_2, n_el_c, n_el_w) %>% select(1:3) %>% group_by(dyn_tot_2,n_el_c) %>% summarise(count = n()) %>%  -->
<!-- ggplot( aes(factor(dyn_tot_2),count,fill = factor(n_el_c )))+ -->
...
fam_ls_w <- ggplot(dyn_ae_ge_fam_uniq)+
  geom_density(aes(x=life_span+5, y = ..density..),size = 1, color = 'black')+
  geom_vline(aes(xintercept = median(dyn_ae_ge_fam_uniq$life_span), color = "median"))+
  geom_vline(aes(xintercept = mean(dyn_ae_ge_fam_uniq$life_span), color = "mean",show.legend  =TRUE))+
   scale_color_manual(name = "statistics", values = c(median = "red", mean = "blue"))+
  scale_y_continuous(
  labels = scales::number_format(accuracy = 0.01))+
 theme_bw()+
  labs(subtitle = "Life span of families", x= "", y = "Density")
...
dyn_ae_ge_fam_uniq$life_span_1 <- dyn_ae_ge_fam_uniq$max_yr- dyn_ae_ge_fam_uniq$`0`+ dyn_ae_ge_fam_uniq$term_duration


medx <- median(dyn_ae_ge_fam_uniq$life_span_1)
meanx <- mean(dyn_ae_ge_fam_uniq$life_span_1)

fam_ls_wo <- ggplot(dyn_ae_ge_fam_uniq)+
  geom_density(aes(x=life_span_1, y = ..density..),size = 1, color = 'black')+
  geom_vline(aes(xintercept =median(dyn_ae_ge_fam_uniq$life_span_1) , color = "median"))+
  geom_vline(aes(xintercept = mean(dyn_ae_ge_fam_uniq$life_span_1), color = "mean",show.legend  =TRUE))+
   scale_color_manual(name = "statistics", values = c(median = "red", mean = "blue"))+
  scale_y_continuous(
  labels = scales::number_format(accuracy = 0.01))+
 theme_bw()+
  labs(subtitle = "family life span after second member", x= "Years", y = "")
...
```{r}
dyn_non_fam_w <- dyn_ae_ge %>% filter(dyn_tot_2==0 &n_elections_won >=1)%>% group_by(family_id) %>% summarise(max_year = max(year), min_year = min(year)) 



term_duration <- dyn_ae_ge %>% filter(dyn_tot_2==0 &n_elections_won >=1)%>% arrange(-year)%>% distinct(family_id, .keep_all = TRUE) %>% select(term_duration)

dyn_non_fam_w <- cbind(dyn_non_fam_w,term_duration )

dyn_non_fam_w <- dyn_non_fam_w%>% mutate(life_span = max_year-min_year+term_duration) 
...
ind_ls <-  ggplot(dyn_non_fam_w)+
  geom_density(aes(x=life_span, y = ..density..),size = 1, color = 'black')+
   labs(subtitle = "Life span of non-families", x= "", y = "")+
  xlim(0,50)+
  ylim(0,.1)+
  geom_vline(aes(xintercept = median(dyn_non_fam_w$life_span), color = "median"))+
  geom_vline(aes(xintercept = mean(dyn_non_fam_w$life_span), color = "mean",show.legend  =TRUE))+
   scale_color_manual(name = "statistics", values = c(median = "red", mean = "blue"))+
  scale_y_continuous(
  labels = scales::number_format(accuracy = 0.01))+
 theme_bw()
...
# dyn_ae_ge %>% filter(family_id =="f0004207")
```
...
year_entry <- ggplot(fam_tab %>% filter(diff>=0), aes(diff))+
  ...
  #xlim(0,45)
  #ylim(.0,.15)+
  geom_vline(aes(xintercept =  median(fam_tab$diff), color = "median"))+
  geom_vline(aes(xintercept =  mean(fam_tab$diff), color = "mean",show.legend  =TRUE))+
   scale_color_manual(name = "statistics", values = c(median = "red", mean = "blue"))+
  scale_y_continuous(
  labels = scales::number_format(accuracy = 0.01))+
 theme_bw()
...
figure <- ggpubr::ggarrange(fam_ls_w, ind_ls, year_entry,fam_ls_wo ,common.legend = TRUE, legend = "bottom", align = "hv")
...
dyn_non_fam <- dyn_ae_ge %>% filter(dyn_tot_2==0)%>% group_by(family_id) %>% summarise(max_year = max(year), min_year = min(year)) %>% mutate(life_span = max_year-min_year) 


medx <- median(dyn_non_fam$life_span)
meanx <- mean(dyn_non_fam$life_span)
...
ggplot(dyn_non_fam)+
  ...
  theme_minimal_hgrid()+
  labs(title = "Life span of non-families", x= "Year", y = "Density")+
  theme(plot.margin = unit(c(.5,.5,.5,.5), "cm"),
        plot.background = element_blank(),
        #plot.title = element_text(hjust = 0.5, size = 20, face = "bold", family = "serif",
                                 # margin = margin(t = 0, r = 0, b = 20, l = 0))
...
#dyn_ae_ge_fam_uniq %>% select(family_id, fam_exp_tot)

medx <- median(dyn_ae_ge_fam_uniq$fam_exp_tot)
meanx <- mean(dyn_ae_ge_fam_uniq$fam_exp_tot)

ggplot(dyn_ae_ge_fam_uniq)+
  geom_density(aes(x=fam_exp_tot, y = ..density..),size = 1, color = 'black')+
  geom_vline(xintercept = medx, color = "red")+
  geom_vline(xintercept = meanx, color = "blue")+
  geom_text(aes(x=(medx-3), label="Median", y=0.01), colour="red", size =4, angle = 90 )+
  geom_text(aes(x=(meanx +3), label="Mean", y = 0.01), colour="blue", size = 4, angle = 90)+
  theme_minimal_hgrid()+
  labs(title = "Political experience of families", x= "Year", y = "Density")+
  theme(plot.margin = unit(c(.5,.5,.5,.5), "cm"),
        plot.background = element_blank(),
        #plot.title = element_text(hjust = 0.5, size = 20, face = "bold", family = "serif",
                                 # margin = margin(t = 0, r = 0, b = 20, l = 0))
...
```{r}
dyn_non_fam <- dyn_ae_ge %>% filter(dyn_tot_2==0)


medx <- median(dyn_non_fam$fam_exp_tot)
meanx <- mean(dyn_non_fam$fam_exp_tot)
...
ggplot(dyn_non_fam)+
  ...
  geom_text(aes(x=(meanx +3), label="Mean", y = 0.01), colour="blue", size = 4, angle = 90)+
  theme_minimal_hgrid()+
  labs(title = "Political experience of non-families", x= "Year", y = "Density")+
  theme(plot.margin = unit(c(.5,.5,.5,.5), "cm"),
        plot.background = element_blank(),
        #plot.title = element_text(hjust = 0.5, size = 20, face = "bold", family = "serif",
                                 # margin = margin(t = 0, r = 0, b = 20, l = 0))
...
#  dyn_ae_ge$school <- ifelse(is.na(dyn_ae_ge$school),"",dyn_ae_ge$school )
# # 
# # 
# dyn_ae_ge$college <- ifelse(is.na(dyn_ae_ge$college),"",dyn_ae_ge$college )
...
ggsave("D:/cpr/thesis/figures/fig_3_9.png", p3_9,width = 5, height =3.5, dpi = 300, units ="in" )
...
ggplot(dyn_indus_ae_ge_uniq,aes(indus_count ,fill = dyn_tot_2_text))+

  geom_density( adjust =3, alpha = .5)+
  ylim(0,.7)+
  ...
  # 
  # ggplot(dyn_indus_ae_ge_uniq)+
  # geom_density(aes(x=indus_count, y = ..density..),size = .7, color = 'black')+
  #   geom_smooth()
...
rt <- dyn_indus_ae_ge_uniq   %>% filter(indus_rt != "0") %>% group_by(dyn_tot_2_text, indus_rt) %>% summarise(count = n()) %>% group_by(dyn_tot_2_text) %>% mutate(sum = sum(count), prop = count/sum) %>% select(dyn_tot_2_text, indus_rt, prop)
...
```{r}
ggplot(rt,aes(x = dyn_tot_2_text, y = prop, fill = indus_rt, label = round(prop,2)))+
  geom_bar(position = "stack", stat= "identity")+
      geom_text(size = 2.5, position = position_stack(vjust = 0.5))+
    colorspace::scale_fill_discrete_qualitative(palette= "dark3" )+
  coord_fixed(ratio =4)+
    theme_minimal()+
labs(title = "Industry composition among dynasts and non-dynasts ",x = "", y = "Proportion of industry")+
    theme(plot.margin = unit(c(.5,.5,.5,.5), "cm"))+
  theme(plot.background = element_blank(),
        axis.line.x = element_line(),
        panel.grid.major = element_blank(),
...
## Caste {.tabset .tabset-fade .tabset-pills}
...
dyn_ae_ge %>% filter(position ==1)%>% group_by(caste_groups) %>% summarise(count = n(), prop = count/length(dyn_ae_ge$caste_groups[dyn_ae_ge$position==1])*100) %>% mutate(caste = "caste") %>% 
...
labs(title = "Caste composition of winners ",x = "", y = "", fill = "Caste Groups")+
    theme(plot.margin = unit(c(.5,.5,.5,.5), "cm"))+
  theme(plot.background = element_blank(),
        axis.line.x = element_line(),
        panel.grid.major = element_blank(),
...
dyn_ae_ge_caste <- dyn_ae_ge %>% filter(fam_exp_tot > 1)
...
ggplot(caste_dyn_ae_ge, aes(x = as.factor(dyn_tot_2), y = prop , fill = caste_groups, label  =ifelse(prop*100>4,round(prop*100,0),"")))+
    geom_bar(stat= "identity") +
    geom_text(size = 2.5, position = position_stack(vjust = 0.5))+
...
### Year sections - caste composition



```{r}

dyn_ae_ge %>% filter(year>=1977)%>%  mutate(period = case_when(year >=1977 & year <= 1984 ~"1974-84",
                                                                                          year>1984 & year<=2009 ~"1989-09",
                                                                                          year >2009 & year <2020 ~"2014-19",
                                                                                          TRUE ~"Others")) %>% 
  group_by(election_type, period, caste_groups) %>% summarise(count = n()) %>% group_by(election_type, period) %>%  mutate(sum = sum(count), pc = round((count/sum)*100),0) %>% 
  ggplot(aes(period, pc, fill = caste_groups))+
  #geom_area()+
  geom_bar(position = "fill", stat = "identity")+
  facet_wrap(~election_type)+
  geom_text(aes(label =pc ), position = position_fill(vjust = 0.5), size = 2.5)+
      theme_minimal()+
labs(title = "Caste composition of MLAs and MPs ",x = "", y = "", fill = "Caste Groups", subtitle = "in percentage")+
    theme(plot.margin = unit(c(.5,.5,.5,.5), "cm"))+
  colorspace::scale_fill_discrete_qualitative(palette= "dark3" )+
  theme(plot.background = element_blank(),
        axis.line.x = element_line(),
...
### Caste composition - wrt year -Area chart

```{r}

dyn_ae_ge %>% filter(year>=1977) %>% 
  group_by(election_type, year, caste_groups) %>% summarise(count = n()) %>% group_by(election_type, year) %>%  mutate(sum = sum(count), pc = count/sum*100) %>% 
  ggplot(aes(year, pc, fill = caste_groups))+
  geom_area()+
  #geom_bar(position = "fill", stat = "identity")+
  facet_wrap(~election_type)+
  #geom_text(aes(label =pc ), position = position_fill(vjust = 0.5), size = 2.5)+
      theme_minimal()+
labs(title = "Caste composition of MLAs and MPs ",x = "", y = "", fill = "Caste Groups")+
    theme(plot.margin = unit(c(.5,.5,.5,.5), "cm"))+
  colorspace::scale_fill_discrete_qualitative(palette= "dark3" )+
  theme(legend.position = "bottom",
  plot.background = element_blank(),
...
<!-- dyn_ae_ge %>% filter(position==1) %>%  -->
<!--   ggplot(aes(factor(dyn_cum_2), turnout_percentage))+ -->
<!--   geom_point()+ -->
<!--   #geom_jitter()+ -->
