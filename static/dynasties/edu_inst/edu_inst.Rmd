...
fit.linear <- lm(data = dyn_up_college, n_colleges ~ dyn_cum_2+term_duration+caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim  +factor(year_el ))
...
fit.poisson <- glm(data = dyn_up_college, n_colleges ~ dyn_cum_2+term_duration+caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim  +factor(year_el ), family =  "quasipoisson")
...
# fit.lm <- lm(data = dyn_up_college_91, n_college ~ dyn_cum_2+ factor(year_el ))
# 
# 
# ()


fit.linear <- lm(data = dyn_up_college_91, n_colleges ~ dyn_cum_2+term_duration+caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim  +factor(year_el ))

fit.logit <- glm(data = dyn_up_college_91, n_colleges ~ dyn_cum_2+caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim  +factor(year_el ))


#summary(fit.logit)


fit.poisson <- glm(data = dyn_up_college_91, n_colleges ~ dyn_cum_2+term_duration+caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim  +factor(year_el ), family =  "quasipoisson")
...
fit.linear <- lm(data = dyn_up_college_91, n_private ~ dyn_cum_2+caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim  +factor(year_el ))

fit.logit <- glm(data = dyn_up_college_91, n_private ~ dyn_cum_2+caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim  +factor(year_el ))


#summary(fit.logit)


fit.poisson <- glm(data = dyn_up_college_91, n_private ~ dyn_cum_2+caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim  +factor(year_el ), family =  "quasipoisson")


#summary(fit.poisson)

dyn_up_college_91$n_college_bin <- ifelse(dyn_up_college_91$n_private ==0,0,1)
...
fe1 = fepois(n_colleges ~ dyn_cum_2+ incumbent+margin_percentage+ turnout_percentage+enop+ term_duration+n_schools+ nl_tot+constituency_type+log(electors)+no_terms|year_el + district_name, data = dyn_up_college_nl_sc,cluster = ~ district_name)

 # summary(fe1) %>% kable() %>% kabl
...
etable(fe1) %>% kable(caption = "UP colleges - Poisson model") %>% kable_styling(full_width = F)
...
<!-- fit.poisson <- glm(data = dyn_up_college, n_colleges ~ dyn_cum_3+term_duration+caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim  +factor(year_el ), family =  "quasipoisson") -->
...
# shrug_adr_all<- fread("D:/cpr/data/shrug/shrug/shrug-v1.4.samosa-affidavits-csv/affidavits_clean.csv")
# 
# shrug_adr <- shrug_adr_all %>% filter(grepl("-09-", ac_id))
# 
# 
# shrug_adr <- shrug_adr %>% mutate(ac_no = as.numeric(str_sub(ac_id, -3)), .after = ac_id)
# 
# shrug_adr_up <- shrug_adr %>% filter(year %in% c(2007, 2012, 2017) & winner ==1)

# 
# adr_12 <- fread("D:/cpr/UP/up-dynasties/dyn_other_data/adr/up_12_adr.csv")
# 
# adr_12 <- adr_12 %>% filter(Position ==1)%>% select(c(Year, Constituency_No,12:12,23:23,30:30,75:76))
# 
# 
# 
# adr_17 <- fread("D:/cpr/UP/up-dynasties/dyn_other_data/adr/up_17_adr.csv")
# 
# adr_17 <- adr_17 %>% filter(Position ==1)%>% select(c(Year, Constituency_No,8:8,19:19,26:26,71:72))
# 
# write.csv(adr_17,"D:/cpr/UP/up-dynasties/dyn_other_data/adr/up_17_adr_s.csv")
# 
# write.csv(adr_12,"D:/cpr/UP/up-dynasties/dyn_other_data/adr/up_12_adr_s.csv")


# rbindlist(list(adr_12, adr_17), use.names = TRUE, fill = TRUE)
# 
# rbind(adr_12, adr_17)
...
fit.poisson <- glm(data = dyn_up_college_adr, n_colleges ~ dyn_cum_2+caste_uc+ caste_yadav+caste_non_yadav_obc + caste_dalit + caste_muslim  + log(total_immovable_assets_totals) + log(total_movable_assets_totals)+ serious_crime+non_serious_crime+
                     factor(year_el ), family =  "quasipoisson")


#summary(fit.poisson)

stargazer(fit.poisson,title="Regression Results - UP  colleges: adr variables - Dynast 2 ",type = "html",
align=TRUE,  no.space=TRUE, keep = c("dyn_cum_2" ,         "caste_uc", "caste_yadav"   ,      "caste_non_yadav_obc" ,"caste_dalit"   ,"caste_muslim" ,"serious_crime ","non_serious_crime","log(total_movable_assets_totals)","log(total_immovable_assets_totals)" ),omit.stat=c("LL","ser","f"),add.lines=list(c('Year fixed effects', 'Yes','Yes','Yes','Yes' )))
