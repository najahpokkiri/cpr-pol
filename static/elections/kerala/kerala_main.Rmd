...
```{r}
ggplot(kl_el %>% filter(front %in% c("UDF","LDF","NDA")), aes(ur_p,vote_share_percentage , color = front))+
  geom_point(alpha = .5)+
  geom_smooth(se = FALSE, method = "lm")+
    scale_color_manual(values = c("LDF" = "red",
                                ...
                                "UDF" = "steelblue",
                                "NDA" = "darkorange2")) +
     labs(title = "Front vote share as a function of  % of urban constituents ", y = "Vote share(%)",subtitle = "Kerala elections - 2001:19", x = "Urban constituents (%) ", color = "Front")+
    theme_minimal()+
  theme(#plot.margin = unit(c(.5,.5,.5,.5), "cm"),
        #legend.position = "bottom",
        plot.background = element_blank(),
...
```{r}
ggplot(kl_el %>% filter(front %in% c("UDF","LDF","NDA")), aes(sc_p,vote_share_percentage , color = front))+
  geom_point(alpha = .5)+
  geom_smooth(se = FALSE, method = "lm")+
    scale_color_manual(values = c("LDF" = "red",
...
kl_el %>%filter(front %in% c("LDF","UDF","NDA")) %>% group_by(year,sub_region.y, front) %>% summarise(vs = sum(votes)/votes_reg*100) %>%distinct(year, sub_region.y, front, .keep_all=TRUE) %>% 
  ggplot(aes(sub_region.y,vs, fill = factor(front,levels = c("NDA","LDF","UDF")), label = round(vs, 0) ))+
  geom_bar(stat = "identity",position = "stack")+
  ...
  facet_wrap(~year)+
    scale_fill_manual(values = c(
                                ...
                                "NDA" = "darkorange1"))+
labs(title = "Vote share of major fronts",subtitle = "2011:2019",  x = "Years", y = "Vote share (%)", fill = "Election Type" )+
  ...
  theme(#plot.margin = unit(c(.5,.5,.5,.5), "cm"),
        ...
        #axis.text.x = element_text(angle = 90, face = "italic", size = 9),
        axis.title = element_text(vjust = -1, size = 15),        
        axis.ticks = element_blank(),
        panel.grid.major = element_line(color = "gray50", size = .5),
        panel.grid.minor = element_blank(),
...
kl_el %>%group_by(year,sub_region.y, party_1) %>% summarise(vs = sum(votes)/votes_reg*100) %>%distinct(year, sub_region.y, party_1, .keep_all=TRUE) %>% filter(party_1 %in% c("BJP", "CPI","CPM","INC","KEC","IUML", "IND" ) & vs>5) %>% 
  ggplot(aes(sub_region.y,vs, fill = party_1, label = round(vs, 0) ))+
  geom_bar(stat = "identity",position = "stack")+
  geom_text(position= position_stack(vjust = .5), size = 3)+
  scale_x_discrete(labels= sub)+
  facet_wrap(~year)+
    scale_fill_manual(values = c(
                                ...
                                "BJP" = "darkorange1",
                                "IND" = "skyblue"))+
labs(title = "Vote share of major parties",subtitle = "2011:2019",  x = "Years", y = "Vote share (%)", fill = "Election Type" )+
